...
fs.writeFileSync('data/presets/categories.json',
    JSON.stringify({ categories: categories }, null, 4));
fs.writeFileSync('data/presets/fields.json',
    JSON.stringify({ fields: fields }, null, 4));
fs.writeFileSync('data/presets/presets.json',
...
var imagery = YAML.load(
    fs.readFileSync('node_modules/editor-layer-index/i18n/en.yaml', 'utf8'));
...
function generateCategories() {
    var categories = {};
    glob.sync(__dirname + '/data/presets/categories/*.json').forEach(
        function(file) {
            var field = read(file),
...
function generateFields() {
    var fields = {};
    glob.sync(__dirname + '/data/presets/fields/**/*.json').forEach(
        function(file) {
            var field = read(file),
                id = stripLeadingUnderscores(
                    file.match(/presets\/fields\/([^.]*)\.json/)[1]);
    ...
    for (var key in suggestions) {
        for (var value in suggestions[key]) {
            for (var name in suggestions[key][value]) {
                ...
                if (!existing[name]) {
                    tags = _.extend({name: name.replace(/"/g, '')},
                        suggestions[key][value][name].tags);
...
function generatePresets() {
    ...
    glob.sync(__dirname + '/data/presets/presets/**/*.json').forEach(
        function(file) {
            var preset = read(file),
                id = stripLeadingUnderscores(
                    file.match(/presets\/presets\/([^.]*)\.json/)[1]);
...
function generateTranslations(fields, presets) {
    ...
    _.forEach(translations.fields, function(field, id) {
        var f = fields[id];
        if (f.keys) {
            field['label#'] = _.each(f.keys).map(
                function(key) { return key + '=*'; }
            ).join(', ');
            if (!_.isEmpty(field.options)) {
                _.each(field.options, function(v,k) {
                    if (id === 'access') {
                        field.options[k]['title#'] =
                        field.options[k]['description#'] = 'access=' + k;
                    } else {
    ...
    _.forEach(translations.presets, function(preset, id) {
        var p = presets[id];
        if (!_.isEmpty(p.tags))
            preset['name#'] = _.toPairs(p.tags).map(
                function(pair) { return pair[0] + '=' + pair[1]; }
            ).join(', ');
        if (p.searchable !== false) {
            if (p.terms && p.terms.length)
                preset['terms#'] = 'terms: ' + p.terms.join();
            preset.terms = '<translate with synonyms or related terms for \'' +
                preset.name + '\', separated by commas>';
        } else {
            delete preset.terms;
        }
    });
        ...
        if (category.members) {
            category.members.forEach(function(preset) {
                if (presets[preset] === undefined) {
                    console.error('Unknown preset: ' + preset + ' in category '
                        + category.name);
                    process.exit(1);
                }
            });
        ...
        if (preset.fields) {
            preset.fields.forEach(function(field) {
                if (fields[field] === undefined) {
                    console.error('Unknown preset field: ' + field +
    ...
    _.forEach(defaults.defaults, function (members, name) {
        members.forEach(function (id) {
            if (!presets[id] && !categories[id]) {
                console.error('Unknown category or preset: ' + id +
                    ' in default ' + name);
                process.exit(1);
            }
        });
...
function translationsToYAML(translations) {
    // comment keys end with '#' and should sort immediately before their
    // related key.
    ...
    return YAML.safeDump(
      { en: { presets: translations }},
      { sortKeys: commentFirst, lineWidth: -1 }
    ).replace(/\'.*#\':/g, '#');
}
