...
var sourceCore = yaml.load(fs.readFileSync('./data/core.yaml', 'utf8')),
    ...
    sourceImagery = yaml.load(fs.readFileSync(
        './node_modules/editor-layer-index/i18n/en.yaml', 'utf8'));
    ...
    asyncMap(Object.keys(locale),
        function(code, done) {
            if (code === 'en' || _.isEmpty(locale[code])) {
                ...
                fs.writeFileSync(outdir + code + '.json',
            ...
            if (!err) {
                fs.writeFileSync('data/locales.json',
                    stringify({ dataLocales: dataLocales }, { space: 4 }));
            }
        }
    );
});
