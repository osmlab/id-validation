...
export function actionCircularize(wayId, projection, maxAngle) {
    ...
    var action = function(graph, t) {
        ...
        var nodes = _.uniq(graph.childNodes(way)),
            ...
            centroid = (points.length === 2)
                ? geoInterp(points[0], points[1], 0.5)
                : d3polygonCentroid(points),
            radius = d3.median(points, function(p) {
                return geoEuclideanDistance(centroid, p);
            }),
        ...
        if (keyNodes.length === 1) {
            var index = nodes.indexOf(keyNodes[0]),
                oppositeIndex =
                    Math.floor((index + nodes.length / 2) % nodes.length);
        ...
        for (var i = 0; i < keyPoints.length; i++) {
            ...
            keyPoints[i] = [
                centroid[0] + (keyPoints[i][0] - centroid[0]) / distance *
                    radius,
                centroid[1] + (keyPoints[i][1] - centroid[1]) / distance *
                    radius
            ];
            ...
            startAngle = Math.atan2(
                keyPoints[i][1] - centroid[1],
                keyPoints[i][0] - centroid[0]
            );
            endAngle = Math.atan2(
                keyPoints[nextKeyNodeIndex][1] - centroid[1],
                keyPoints[nextKeyNodeIndex][0] - centroid[0]
            );
            ...
            if (indexRange === 1 && inBetweenNodes.length) {
                ...
                _.each(_.without(
                    graph.parentWays(keyNodes[i]), way),
                    function(sharedWay) {
                        if (sharedWay.areAdjacent(startNode.id, endNode.id)) {
                            var startIndex2 = sharedWay.nodes.lastIndexOf(
                                    startNode.id),
                                endIndex2 = sharedWay.nodes.lastIndexOf(
                                    endNode.id),
                            ...
                            for (j = 0; j < inBetweenNodes.length; j++) {
                                sharedWay = sharedWay.addNode(
                                    inBetweenNodes[j], insertAt + j);
                            }
        ...
        for (var i = 0; i < hull.length - 1; i++) {
            ...
            for (var j = 1; j < indexRange; j++) {
                var point = geoInterp(hull[i], hull[i+1], j / indexRange),
                    node = nodes[(j + startIndex) % nodes.length].move(
