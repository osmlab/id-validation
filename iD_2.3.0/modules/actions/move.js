...
export function actionMove(moveIds, tryDelta, projection, cache) {
    ...
    function setupCache(graph) {
        function canMove(nodeId) {
            ...
            // Restrict movement of a vertex where >2 ways meet, unless all
            // parentWays are moving too..
            var parentsMoving = _.every(parents,
                function(id) { return cache.moving[id]; });
            ...
            ids.forEach(function(id) {
                ...
                childNodes.forEach(function(node) {
                    ...
                    var moved = graph.entity(id),
                        unmoved = _.find(parents,
                            function(way) { return !cache.moving[way.id]; });
                    if (!unmoved) return;

                    // exclude ways that are overly connected..
                    if (_.intersection(moved.nodes, unmoved.nodes).length > 2)
                        return;
    ...
    function unZorroIntersection(intersection, graph) {
        ...
        var edge1 = !isEP1 && geoChooseEdge(nodes1, projection(vertex.loc),
                projection),
            edge2 = !isEP2 && geoChooseEdge(nodes2, projection(vertex.loc),
                projection),
            loc;
