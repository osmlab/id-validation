...
export function actionOrthogonalize(wayId, projection) {
    ...
    var action = function(graph, t) {
        ...
        var way = graph.entity(wayId),
            ...
            points = _.uniq(nodes).map(
                function(n) { return projection(n.loc); }),
            corner = {i: 0, dotp: 1},
            epsilon = 1e-4,
        ...
        if (points.length === 3) {   // move only one vertex for right triangle
            for (i = 0; i < 1000; i++) {
                ...
                points[corner.i] = addPoints(
                    points[corner.i], motions[corner.i]);
                score = corner.dotp;
                if (score < epsilon) {
                    break;
                }
        ...
        function calcMotion(b, i, array) {
            ...
            scale = 2 * Math.min(geoEuclideanDistance(p, [0, 0]),
                geoEuclideanDistance(q, [0, 0]));
            p = normalizePoint(p, 1.0);
            q = normalizePoint(q, 1.0);

            dotp = filterDotProduct(p[0] * q[0] + p[1] * q[1]);

            // nasty hack to deal with almost-straight segments (angle is closer
    ...
    function squareness(points) {
        return points.reduce(function(sum, val, i, array) {
            ...
            return sum + 2.0 * Math.min(Math.abs(dotp - 1.0),
    ...
    function filterDotProduct(dotp) {
        if (lowerThreshold > Math.abs(dotp) ||
            Math.abs(dotp) > upperThreshold) {
            return dotp;
        }
