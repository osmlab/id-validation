...
/*
  Order the nodes of a way in reverse order and reverse any direction dependent
  tags other than `oneway`. (We assume that correcting a backwards oneway is the
  primary reason for reversing a way.)
  ...
  The following transforms are performed:
   ...
   The JOSM implementation was used as a guide, but transformations that were of
   unclear benefit or adjusted tags that don't seem to be used in practice were
   omitted.

   Also, each node on the way is examined for its own tags and the following
...
export function actionReverse(wayId, options) {
    ...
    function reverseDirectionTags(node) {
        // Update the direction based tags as appropriate then return an updated
        // node
        return node.update({
            tags: _.transform(node.tags, function(acc, tagValue, tagKey) {
                // See if this is a direction tag and reverse (or use existing
                // value if not recognised)
                if (tagKey === 'direction') {
                    acc[tagKey] = {forward: 'backward', backward: 'forward',
                        left: 'right', right: 'left'}[tagValue] || tagValue;
                } else {
                    // Use the reverseKey method to cater for situations such as
                    // traffic_sign:forward=stop This will pass through other
                    // tags unchanged
    ...
    function reverseTagsOnNodes(graph, nodeIds) {
        // Reverse the direction of appropriate tags attached to the nodes
        // (#3076)
        return _(nodeIds)
            ...
            // Chain together consecutive updates to the graph for each updated
            // node and return
            .reduce(
    ...
    return function(graph) {
        ...
        graph.parentRelations(way).forEach(function(relation) {
            relation.members.forEach(function(member, index) {
                if (member.id === way.id &&
                    (role = roleReversals[member.role])) {
        ...
        // Reverse any associated directions on nodes on the way and then
        // replace the way itself with the reversed node ids and updated way
        // tags
