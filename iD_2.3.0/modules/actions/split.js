...
import { osmIsSimpleMultipolygonOuterMember, osmRelation, osmWay }
    from '../osm/index';
...
export function actionSplit(nodeId, newWayIds) {
    ...
    function splitArea(nodes, idxA, graph) {
        ...
        function dist(nA, nB) {
            return geoSphericalDistance(
                  graph.entity(nA).loc, graph.entity(nB).loc);
        }
            ...
            if (relation.isRestriction()) {
                ...
                if (via && wayB.contains(via.id)) {
                    relation = relation.updateMember(
                          {id: wayB.id}, relation.memberById(wayA.id).index);
    ...
    action.ways = function(graph) {
        var node = graph.entity(nodeId),
            ...
            hasLines = _.some(parents, function(parent) {
    ...
    action.disabled = function(graph) {
        ...
        if (candidates.length === 0 ||
            (wayIds && wayIds.length !== candidates.length))
            return 'not_eligible';
    };
