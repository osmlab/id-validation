...
export function behaviorDrawWay(context, wayId, index, mode, startGraph) {
    ...
    if (!isArea) {
        startIndex = typeof index === 'undefined'
            ? origWay.nodes.length - 1 : 0;
        start = osmNode({ id: 'nStart', loc:
            context.entity(origWay.nodes[startIndex]).loc });
        end = osmNode({ id: 'nEnd', loc: context.map().mouseCoordinates() });
        segment = osmWay({ id: 'wTemp',
            nodes: typeof index === 'undefined'
                ? [start.id, end.id] : [end.id, start.id],
    ...
    function move(datum) {
        ...
        if (datum.type === 'node' && datum.id !== end.id) {
            ...
            if (trySnap) {
                loc = geoChooseEdge(context.childNodes(datum), context.mouse(),
                    context.projection).loc;
            }
        }
    ...
    function undone() {
        ...
        // Remove initial no-op edit and whatever edit happened immediately
        // before it.
        context.pop(2);
        tempEdits = 0;
    ...
    function AddDrawEntities() {
        return function(graph) {
            if (isArea) {
                ...
                // For line drawing, add a temporary start, end, and segment to
                // the graph. This allows us to class the new segment as
                // `active`, but still connect it back to parts of the way that
    ...
    function ReplaceDrawEntities(newNode) {
        return function(graph) {
            if (isArea) {
                ...
                // `newNode` gets inserted into the _original_ way as the
                // penultimate node.
                return graph
                    .replace(origWay.addNode(newNode.id))
                    .remove(end);
            } else {
                // For line drawing, add the `newNode` to the way at specified
                // index, and remove the temporary start, end, and segment.
                return graph
    ...
    drawWay.add = function(loc) {
        ...
        var last = context.hasEntity(
            origWay.nodes[origWay.nodes.length - (isArea ? 2 : 1)]);
    ...
    drawWay.addNode = function(node) {
        ...
        if (origWay.areAdjacent(
            node.id, origWay.nodes[origWay.nodes.length - 1]))
            return;

        // Clicks should not occur on the drawing node, however a space keypress
        // can sometimes grab that node's datum (before it gets classed as
        // `active`?)  #4016
        if (node.id === end.id) {
    ...
    function RemoveLintHanging(nids) {
        return function(graph) {
            return graph.removeTagsOnMultiEnts(nids, ['lint_hanging']);
        };
    }
    ...
    drawWay.finish = function() {
        ...
        if (origWay.tags.lint_disconnected) {
            // check if origWay now connects to connected ways
            var graph = context.graph(),
                reachables = graph.findReachableWids(wayId),
                hasConnectedRoad = false;
            for (let wid of reachables) {
                var way = graph.entity(wid);
                if (way.tags.highway && !way.tags.lint_disconnected) {
                    hasConnectedRoad = true;
                    break;
                }
            }
            if (hasConnectedRoad) {
                var newTags = _.assign({}, origWay.tags);
                delete newTags['lint_disconnected'];
                context.perform(actionChangeTags(wayId, newTags),
                    t('operations.change_tags.annotation'));
            }
        } else {
            // check if any way with lint_disconnected gets connected to origWay
            var graph = context.graph(),
                reachables = graph.findReachableWids(wayId),
                disconnectedWay = null;
            for (let wid of reachables) {
                var way = graph.entity(wid);
                if (way.tags.highway && way.tags.lint_disconnected) {
                    disconnectedWay = way;
            ...
            if (disconnectedWay !== null) {
                var newTags = _.assign({}, disconnectedWay.tags);
                delete newTags['lint_disconnected'];
                context.perform(actionChangeTags(disconnectedWay.id, newTags),
                    t('operations.change_tags.annotation'));
            }
        }

        // look for lint_hanging tags to remove
        way = context.entity(wayId);
        var nids = [];
        for (var i = 0; i < way.nodes.length; i++) {
            var nid = way.nodes[i],
                node = context.entity(nid);
            if (!node.tags.lint_hanging) continue;
            if ((i > 0 && i < way.nodes.length - 1) ||
                context.graph()._parentWays[nid].length > 1) {
                nids.push(nid);
            }
        }
        if (nids.length > 0) {
            context.perform(RemoveLintHanging(nids),
                t('operations.change_tags.annotation'));
        }
