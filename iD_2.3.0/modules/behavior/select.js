...
export function behaviorSelect(context) {
    ...
    function click() {
        ...
        var isMultiselect = d3.event.shiftKey ||
                d3.select('#surface .lasso').node(),
            isShowAlways = +context.storage('edit-menu-show-always') === 1,
            datum = d3.event.target.__data__ ||
                (lastMouse && lastMouse.target.__data__),
            mode = context.mode();
        ...
        if (!(datum instanceof osmEntity)) {
            ...
            if (!isMultiselect) {
                if (selectedIDs.length > 1 &&
                    ...
                    context.enter(modeSelect(context, [datum.id]).suppressMenu(
                        suppressMenu));
                }
                ...
                if (selectedIDs.indexOf(datum.id) !== -1) {
                    ...
                    if (!suppressMenu && !isShowAlways) {
                        ...
                        // deselect clicked entity, then reenter select mode or
                        // return to browse mode..
                        selectedIDs = _.without(selectedIDs, datum.id);
                        context.enter(selectedIDs.length
                            ? modeSelect(context, selectedIDs)
                            : modeBrowse(context)
                        );
                    }
                } else {
                    ...
                    context.enter(modeSelect(context, selectedIDs).suppressMenu(
                        suppressMenu));
                }
            }
        }
