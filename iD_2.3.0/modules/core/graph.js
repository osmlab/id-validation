...
export function coreGraph(context, other, mutable) {
    if (!(this instanceof coreGraph)) {
        return new coreGraph(context, other, mutable);
    }

    if (other instanceof coreGraph) {
        var base = other.base();
        this.entities = _.assign(Object.create(base.entities), other.entities);
        this._parentWays = _.assign(Object.create(base.parentWays),
            other._parentWays);
        this._parentRels = _.assign(Object.create(base.parentRels),
            other._parentRels);
...
coreGraph.prototype = {
    ...
    // remove lint_disconnected from roads reachable from wid
    removeReachableDisconnectedTag: function(wid) {
        var reachables = this.findReachableWids(wid),
            targetWids = [];
        for (let wid of reachables) {
            var way = this.entities[wid];
            if (way.tags['lint_disconnected']) {
               targetWids.push(wid);
            }
        }
        return this.removeTagsOnMultiEnts(targetWids, ['lint_disconnected']);
    },


    // find IDs of reachable roads from wid
    findReachableWids: function(wid) {
        var queue = [wid],
            reachables = new Set();
        while (queue.length > 0) {
            var wid1 = queue.shift(),
                way = this.entities[wid1];
            for (var i = 0; i < way.nodes.length; i++) {
                var nid = way.nodes[i],
                    pwids = this._parentWays[nid];
                if (!pwids || pwids.length < 2) continue;
                for (var j = 0; j < pwids.length; j++) {
                    var pwid = pwids[j],
                        pway = this.entities[pwid];
                    if (!pway.tags.highway) continue;
                    if (pwid !== wid && !reachables.has(pwid)) {
                        reachables.add(pwid);
                        queue.push(pwid);
                    }
                }
    ...
    rebase: function(entities, stack, force) {
        ...
        for (i = 0; i < entities.length; i++) {
            ...
            // Restore provisionally-deleted nodes that are discovered to have
            // an extant parent
            if (entity.type === 'way') {
            ...
            if (base.parentWays[child]) {
                for (k = 0; k < base.parentWays[child].length; k++) {
                    ...
                    if (!this.entities.hasOwnProperty(id)
                        && !_.includes(this._parentWays[child], id)) {
        ...
        for (i = 0; i < keys.length; i++) {
            ...
            if (base.parentRels[child]) {
                for (k = 0; k < base.parentRels[child].length; k++) {
                    ...
                    if (!this.entities.hasOwnProperty(id)
                        && !_.includes(this._parentRels[child], id)) {
    ...
    // Updates calculated properties (parentWays, parentRels) for the specified
    // change
    _updateCalculated: function(oldentity, entity, parentWays, parentRels) {
        ...
        if (type === 'way') {
            ...
            for (i = 0; i < removed.length; i++) {
                parentWays[removed[i]] = _.without(
                    parentWays[removed[i]], oldentity.id);
            }
            ...
            for (i = 0; i < removed.length; i++) {
                parentRels[removed[i].id] = _.without(
                    parentRels[removed[i].id], oldentity.id);
            }
            for (i = 0; i < added.length; i++) {
    ...
    addNegEntities: function(entities) {
        return this.update(function() {
            for (var eid in entities) {
                if (eid[1] === '-') {
                    this._updateCalculated(this.entities[eid], entities[eid]);
                    this.entities[eid] = entities[eid];
                }
            }
        });
    },

    // update tags on multiple entities given by eids
    updateTagsOnMultiEnts: function(eids, key, value) {
        return this.update(function() {
            for (var i = 0; i < eids.length; i++) {
                var ent = this.entities[eids[i]],
                    newTags = _.assign({}, ent.tags);
                newTags[key] = value;
                this.entities[eids[i]] = ent.update({tags: newTags});
            }
        });
    },

    // remove tags on multiple entities given by eids
    removeTagsOnMultiEnts: function(eids, keys) {
        return this.update(function() {
            for (var i = 0; i < eids.length; i++) {
                var ent = this.entities[eids[i]],
                    newTags = _.assign({}, ent.tags),
    ...
    update: function() {
        var graph = this.frozen ? coreGraph(this.context, this, true) : this;
    ...
    isForXMLEditing: function() {
        return this.context.editInBoundsMode() ===
            this.context.EDIT_IN_BOUNDS_FROM_OSM_XML;
    }
};
