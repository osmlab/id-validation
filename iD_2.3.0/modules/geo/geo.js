    ...
    return points.map(function(point) {
        ...
        return [
            radial[0] * Math.cos(angle) -
                radial[1] * Math.sin(angle) + around[0],
            radial[0] * Math.sin(angle) +
                radial[1] * Math.cos(angle) + around[1]
        ];
    });
}
...
export function geoPolygonIntersectsPolygon(outer, inner, checkSegments) {
   ...
   return testPoints(outer, inner) ||
      (!!checkSegments && testSegments(outer, inner));
}
...
export function geoWayLength(wid, graph) {
    var length = 0,
        way = graph.entity(wid);
