...
export function modeDragNode(context) {
    ...
    var nudgeInterval,
        ...
        lastLoc,
        selectedIDs = [],
        hover = behaviorHover(context).altDisables(true).on('hover',
            context.ui().sidebar.hover),
    ...
    function moveAnnotation(entity) {
        return t('operations.move.annotation.' +
            entity.geometry(context.graph()));
    }


    function connectAnnotation(entity) {
        return t('operations.connect.annotation.' +
            entity.geometry(context.graph()));
    }
    ...
    function start(entity) {
        ...
        // activeIDs generate no pointer events.  This prevents the node or
        // vertex being dragged from trying to connect to itself or its parent
        // element.
    ...
    function doMove(entity, nudge) {
        ...
        var currPoint = (d3.event && d3.event.point)
                || context.projection(lastLoc),
        ...
        if (!nudgeInterval) {
            if (d.type === 'node' && d.id !== entity.id) {
                ...
                !d3.select(d3.event.sourceEvent.target).classed('fill')) {
                loc = geoChooseEdge(context.childNodes(d), context.mouse(),
    ...
    function end(entity) {
        ...
        if (d.type === 'way') {
            var choice = geoChooseEdge(context.childNodes(d), context.mouse(),
                context.projection);
            context.replace(
                actionAddMidpoint(
                    {
