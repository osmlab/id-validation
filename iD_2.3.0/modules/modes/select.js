    ...
    mode.enter = function() {
        ...
        function dblclick() {
            ...
            if (datum instanceof osmWay && !target.classed('fill')) {
                var choice = geoChooseEdge(context.childNodes(datum),
                        context.mouse(), context.projection),
                    prev = datum.nodes[choice.index - 1],
                    next = datum.nodes[choice.index];

                context.perform(
                    actionAddMidpoint(
                        {loc: choice.loc, edge: [prev, next]},
                        osmNode()
                    ),
        ...
        function selectElements(drawn) {
            ...
            var selection = context.surface()
                .selectAll(utilEntityOrMemberSelector(
                    selectedIDs, context.graph())
                );

            if (selection.empty()) {
                // Return to browse mode if selected DOM elements have
                // disappeared because the user moved them out of view..
                var source = d3.event && d3.event.type === 'zoom' &&
                    d3.event.sourceEvent;
                if (drawn && source && (source.type === 'mousemove' ||
        ...
        var operations = _.without(
              d3.values(Operations), Operations.operationDelete
        ...
        if (isRadialMenu) {
            ...
            operations.unshift(
                Operations.operationDelete(selectedIDs, context));
        } else {
