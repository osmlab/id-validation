...
export function operationMerge(selectedIDs, context) {
    ...
    function updatePresetTags(newGraph, ids) {
        ...
        context.replace(
            actionChangePreset(id, null, newPreset), operation.annotation());
    }
    ...
    var join = actionJoin(selectedIDs),
        ...
        hasPosWay = _.some(selectedIDs, function(id) {
            return id[0] === 'w' && id[1] !== '-';
        }),
        isEditingXML = context.editInBoundsMode() ===
    ...
    operation.disabled = function() {
        if (hasPosWay && isEditingXML) return true;
    ...
    operation.tooltip = function() {
        if (hasPosWay && isEditingXML) {
            return t('operations.merge.merge_pos_ways');
        }
