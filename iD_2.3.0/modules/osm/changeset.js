...
_.extend(osmChangeset.prototype, {
    ...
    osmChangeJXON: function(changes) {
        ...
        function sort(changes) {
            ...
            function isNew(item) {
                return !sorted[ item['@id'] ] &&
                    !_.find(processing, function(proc) {
        ...
        return {
            osmChange: {
                '@version': 0.6,
                '@generator': 'iD',
                'create': sort(nest(changes.created.map(rep),
                    ['node', 'way', 'relation'])),
                'modify': nest(changes.modified.map(rep),
                    ['node', 'way', 'relation']),
                'delete': _.extend(nest(changes.deleted.map(rep),
                    ['relation', 'way', 'node']), { '@if-unused': true })
            }
        };
    },
