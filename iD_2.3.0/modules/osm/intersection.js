...
export function osmIntersection(graph, vertexId) {
    ...
    parentWays.forEach(function(way) {
        ...
        if (isAffix && !isClosingNode) {
            ...
            if (isClosingNode) {
                // split at midpoint
                splitIndex = Math.ceil(way.nodes.length / 2);
                wayA = osmWay({id: way.id + '-a', tags: way.tags,
                    nodes: way.nodes.slice(0, splitIndex)});
                wayB = osmWay({id: way.id + '-b', tags: way.tags,
                    nodes: way.nodes.slice(splitIndex)});
                indexA = 1;
                indexB = way.nodes.length - 2;
            } else {
                // split at vertexid
                splitIndex = _.indexOf(way.nodes, vertex.id, 1);
                wayA = osmWay({id: way.id + '-a', tags: way.tags,
                    nodes: way.nodes.slice(0, splitIndex + 1)});
                wayB = osmWay({id: way.id + '-b', tags: way.tags,
                    nodes: way.nodes.slice(splitIndex)});
                indexA = splitIndex - 1;
                indexB = splitIndex + 1;
    ...
    intersection.turns = function(fromNodeId) {
        ...
        function withRestriction(turn) {
            graph.parentRelations(
                graph.entity(turn.from.way)).forEach(function(relation) {
...
export function osmInferRestriction(graph, from, via, to, projection) {
    var fromWay = graph.entity(from.way),
        ...
        fromOneWay =
            (fromWay.tags.oneway === 'yes' && fromWay.last() === via.node) ||
            (fromWay.tags.oneway === '-1' && fromWay.first() === via.node),
        toOneWay =
            (toWay.tags.oneway === 'yes' && toWay.first() === via.node) ||
