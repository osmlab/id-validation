...
_.extend(osmRelation.prototype, {
    ...
    // Return an array of members, each extended with an 'index' property whose
    // value is the member index.
    ...
    // is returned, extended with an 'index' property whose value is the member
    // index.
    ...
    // Return the first member with the given id and role. A copy of the member
    // object is returned, extended with an 'index' property whose value is the
    // member index.
    ...
    removeMembersWithID: function(id) {
        var members = _.reject(this.members,
    ...
    replaceMember: function(needle, replacement) {
        ...
        for (var i = 0; i < this.members.length; i++) {
            ...
            if (member.id !== needle.id) {
                ...
                members.push({id: replacement.id, type: replacement.type,
                    role: member.role});
            }
        }
            ...
            if (this.isMultipolygon()) {
                ...
                return {
                    ...
                    features: this.members.map(function (member) {
                        return _.extend({role: member.role},
    ...
    // Returns an array [A0, ... An], each Ai being an array of node arrays
    // [Nds0, ... Ndsm], where Nds0 is an outer ring and subsequent Ndsi's
    // (if any i > 0) being inner rings.
    //
    // This corresponds to the structure needed for rendering a multipolygon
    // path using a `evenodd` fill rule, as well as the structure of a GeoJSON
    // MultiPolygon geometry.
    //
    // In the case of invalid geometries, this function will still return a
    // result which includes the nodes of all way members, but some Nds may be
    // unclosed and some inner rings not matched with the intended outer ring.
    //
    multipolygon: function(resolver) {
        var outers = this.members.filter(
                function(m) { return 'outer' === (m.role || 'outer'); }),
            inners = this.members.filter(
                function(m) { return 'inner' === m.role; });

        outers = osmJoinWays(outers, resolver);
        inners = osmJoinWays(inners, resolver);

        outers = outers.map(
            function(outer) { return _.map(outer.nodes, 'loc'); });
        inners = inners.map(
            function(inner) { return _.map(inner.nodes, 'loc'); });

        var result = outers.map(function(o) {
            // Heuristic for detecting counterclockwise winding order. Assumes
            // that OpenStreetMap polygons are not hemisphere-spanning.
            return [
                d3.geoArea({ type: 'Polygon', coordinates: [o] }) > 2 * Math.PI
                    ? o.reverse()
                    : o
            ];
        });
        ...
        for (var i = 0; i < inners.length; i++) {
            ...
            if (d3.geoArea({ type: 'Polygon', coordinates: [inner] }) <
                2 * Math.PI) {
                inner = inner.reverse();
            }
