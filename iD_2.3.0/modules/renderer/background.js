...
export function rendererBackground(context) {
    ...
    background.numGridSplits = function(_) {
        if (!arguments.length) return numGridSplits;
        numGridSplits = _;
    ...
    background.updateImagery = function() {
        ...
        overlayLayers
            .filter(function (d) { return !d.source().isLocatorOverlay(); })
            .forEach(function (d) {
                imageryUsed.push(d.source().imageryUsed());
            });
    ...
    background.showsLayer = function(d) {
        return d.id === baseLayer.source().id ||
            overlayLayers.some(function(layer) {
    ...
    background.init = function() {
        ...
        if (!chosen && extent) {
            best = _.find(this.sources(extent),
                function(s) { return s.best(); });
        }

        if (chosen && chosen.indexOf('custom:') === 0) {
            background.baseLayerSource(rendererBackgroundSource.Custom(
                chosen.replace(/^custom:/, '')));
        } else {
            background.baseLayerSource(
                findSource(chosen) || best || findSource('Bing') ||
                backgroundSources[1] || backgroundSources[0]
            );
        }
        ...
        if (q.offset) {
            var offset = q.offset.replace(/;/g, ',').split(',').map(
                    function(n) { return !isNaN(n) && n;}
                );
