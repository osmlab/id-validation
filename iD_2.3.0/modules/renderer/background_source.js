...
export function rendererBackgroundSource(data) {
    ...
    source.description = function() {
        var id_safe = source.id.replace('.', '<TX_DOT>');
        return t('imagery.' + id_safe + '.description',
            { default: description });
    };
    ...
    source.area = function() {
        ...
        var area = d3.geoArea({ type: 'MultiPolygon',
...
rendererBackgroundSource.Bing = function(data, dispatch) {
    ...
    jsonpRequest(url, function(json) {
        providers = json.resourceSets[0].resources[0].imageryProviders.map(
            function(provider) {
                return {
            ...
            if (err) {
                ...
                var vintage = {
                    start: localeDateString(
                        ...
                        result.resourceSets[0].resources[0].vintageEnd)
                };
                vintage.range = vintageRange(vintage);
