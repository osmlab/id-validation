...
export function rendererFeatures(context) {
    ...
    function defineFeature(k, filter, max) {
        ...
        _features[k] = {
            ...
            enable: function() {
                this.enabled = true; this.currentMax = this.defaultMax;
            },
    ...
    // contains a past/future tag, but not in active use as a
    // road/path/cycleway/etc..
    defineFeature('past_future', function isPastFuture(entity) {
        ...
        for (var i = 0; i < strings.length; i++) {
            var s = strings[i];
            if (past_futures[s] || past_futures[entity.tags[s]]) {
                return true;
            }
        }
        return false;
    ...
    _features['landuse'].disable();
    _features['boundaries'].disable();
    _features['power'].disable();
    _features['past_future'].disable();
    _features['others'].disable();
    ...
    features.autoHidden = function(k) {
        if (!arguments.length) {
            return _keys.filter(
                function(k) { return _features[k].autoHidden(); }
            );
        }
    ...
    features.toggle = function(k) {
        if (_features[k]) {
            (function(f) { return f.enabled ? f.disable() : f.enable(); }(
                _features[k])
            );
            update();
        }
    ...
    features.gatherStats = function(d, resolver, dimensions) {
        var needsRedraw = false,
            ...
            entities = [].concat(
                type.relation || [], type.way || [], type.node || []),
        ...
        // adjust the threshold for point/building culling based on viewport
        // size.. a _cullFactor of 1 corresponds to a 1000x1000px viewport..
        ...
        if (!_cache[ent].matches) {
            ...
            for (var i = 0; i < _keys.length; i++) {
                if (_keys[i] === 'others') {
                    ...
                    // If an entity...
                    //   1. is a way that hasn't matched other 'interesting'
                    //      feature rules,
                    //   2. and it belongs to a single parent multipolygon
                    //      relation
                    // ...then match whatever feature rules the parent
                    // multipolygon has matched. see #2548, #2887
                    //
                    // IMPORTANT:
                    // For this to work, getMatches must be called on relations
                    // before ways.
                    //
                    if (entity.type === 'way') {
                        var parents = features.getParents(
                            entity, resolver, geometry);
                        if (parents.length === 1 &&
    ...
    features.hasHiddenConnections = function(entity, resolver) {
        ...
        if (entity.type === 'midpoint') {
            childNodes = [resolver.entity(entity.edge[0]),
                resolver.entity(entity.edge[1])];
            connections = [];
        } else {
            childNodes = entity.nodes ? resolver.childNodes(entity) : [];
            connections = features.getParents(entity, resolver,
        ...
        connections = _.reduce(childNodes, function(result, e) {
            return resolver.isShared(e)
                ? _.union(result, resolver.parentWays(e))
                : result;
        }, connections);
