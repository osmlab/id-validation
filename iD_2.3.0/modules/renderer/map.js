...
import { geoExtent, geoExtentFromBounds } from '../geo/index';
...
export function rendererMap(context) {
    ...
    function map(selection) {
        ...
        context.history()
            ...
            .on('undone.map redone.map', function(stack) {
                ...
                if (Array.isArray(stack.selectedIDs)) {
                    followSelected = (stack.selectedIDs.length === 1 &&
                        stack.selectedIDs[0][0] === 'n');
                    context.enter(
                        modeSelect(context, stack.selectedIDs)
        ...
        // Need a wrapper div because Opera can't cope with an absolutely
        // positioned
        ...
        surface
            ...
            .on('mouseover.vertices', function() {
                if (map.editable() && !transformed) {
                    ...
                    surface.selectAll('.data-layer-osm').call(
            ...
            .on('mouseout.vertices', function() {
                if (map.editable() && !transformed) {
                    var hover = d3.event.relatedTarget &&
                        d3.event.relatedTarget.__data__;
                    surface.selectAll('.data-layer-osm').call(
                        drawVertices.drawHover,
                        context.graph(), hover,
                        map.extent(),
    ...
    function zoomEventFilter() {
        ...
        // This can happen if a previous `mousedown` occurred without a
        // `mouseup`. If we detect this, dispatch `mouseup` to complete the
        // orphaned gesture, so that d3-zoom won't stop propagation of new
        // `mousedown` events.
    ...
    function editOff() {
        ...
        surface.selectAll('.bbox-svg *').remove();
        surface.selectAll('.grids-svg *').remove();
        dispatch.call('drawn', this, {full: true});
    }
    ...
    function zoomPan(manualEvent) {
        ...
        // If wheel delta is provided in LINE units, recalculate it in PIXEL
        // units We are essentially redoing the calculations that occur here:
    ...
    function drawMapBounds() {
        ...
        var d3Path = d3.geoPath().projection(projection),
            mapBounds = context.getMapBounds(),
            minlat = mapBounds.minlat,
            minlon = mapBounds.minlon,
            maxlat = mapBounds.maxlat,
            maxlon = mapBounds.maxlon,
            extent = geoExtentFromBounds(mapBounds),
            bbox = {type: 'Polygon', coordinates: [extent.polygon()]},
            numGridSplits = context.background().numGridSplits();

        var bboxSvg = surface.selectAll('.bbox-svg')
            .data([0]);
        bboxSvg.enter()
            .append('svg')
            .attr('class', 'bbox-svg');

        var bboxPath = bboxSvg.selectAll('.map-bbox')
            .data([bbox]);
        bboxPath.enter()
            .append('path')
            .attr('class', 'map-bbox');
        bboxPath.attr('d', d3Path)
            .classed('thick', true);
        bboxPath.exit()
            .remove();
    ...
    function redraw(difference, extent) {
        ...
        if (context.editInBoundsMode() === context.EDIT_IN_BOUNDS_FROM_URL_PARAM
            &&
            context.dataInBoundsLoadState() !== context.DATA_IN_BOUNDS_LOADED) {
            context.loadInBoundsFromOSMServer();
        }

        // If we are in the middle of a zoom/pan, we can't do differenced
        // redraws.
        // It would result in artifacts where differenced entities are redrawn
        // with one transform and unchanged entities with another.
        if (resetTransform()) {
    ...
    var queueRedraw = _.throttle(redraw, 400);
    ...
    function setTransform(t2, duration, force) {
        ...
        if (duration) {
            _selection
                ...
                .call(
                    zoom.transform,
                    d3.zoomIdentity.translate(t2.x, t2.y).scale(t2.k)
                );
        } else {
    ...
    function setZoom(z2, force, duration) {
        ...
        var k = projection.scale(),
            k2 =
                Math.max(ztok(2), Math.min(ztok(24), ztok(z2))) / (2 * Math.PI),
            center = pxCenter(),
        ...
        if (duration) {
            ...
            _selection
                ...
                .call(
                    zoom.transform,
                    d3.zoomIdentity.translate(t[0], t[1]).scale(k2)
                );
        } else {
    ...
    function setCenter(loc2, duration) {
        ...
        if (duration) {
            _selection
                ...
                .call(
                    zoom.transform,
                    d3.zoomIdentity.translate(t[0], t[1]).scale(k)
                );
        } else {
            projection.translate(t);
            transformStart = projection.transform();
    ...
    map.zoomTo = function(entity, zoomLimits) {
        ...
        map.centerZoom(extent.center(),
            Math.min(Math.max(z2, zoomLimits[0]), zoomLimits[1]));
    };
    ...
    map.editable = function() {
        var isInEditableZoom = map.zoom() >= context.minEditableZoom();
        if (context.editInBoundsMode() !== context.EDIT_IN_BOUNDS_FROM_OSM_XML)
        {
            return isInEditableZoom;
        } else {
            return isInEditableZoom &&
                context.dataInBoundsLoadState() ===
