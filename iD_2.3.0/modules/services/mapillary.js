...
function loadTiles(which, url, projection) {
    ...
    _.filter(which.inflight, function(v, k) {
        var wanted = _.find(tiles,
            function(tile) { return k === (tile.id + ',0'); });
    ...
    cache.inflight[id] = d3.request(nextURL)
        ...
        .get(function(err, data) {
            ...
            for (var i = 0; i < data.features.length; i++) {
                ...
                features.push({minX: loc[0], minY: loc[1], maxX: loc[0],
                    maxY: loc[1], data: d});
            }
...
export default {
    ...
    images: function(projection) {
        ...
        return searchLimited(psize, limit, projection,
            mapillaryCache.images.rtree);
    },
    ...
    signsSupported: function() {
        ...
        if ((detected.browser.toLowerCase() === 'safari')
            && (parseFloat(detected.version) < 10))
            return false;
        return true;
    },
    ...
    loadSigns: function(context, projection) {
        ...
        if (!mapillarySignDefs) {
            mapillarySignSprite = context.asset(
                'img/traffic-signs/traffic-signs.png');
            mapillarySignDefs = {};
            d3.json(
                context.asset('img/traffic-signs/traffic-signs.json'),
                function(err, data) {
                    if (err) return;
    ...
    initViewer: function(imageKey, context) {
        ...
        function nodeChanged(node) {
            ...
            if (index > -1) { // nodechange initiated from clicking on a marker
                ...
                var loc = node.computedLatLon
                    ? [node.computedLatLon.lon, node.computedLatLon.lat]
                    : [node.latLon.lon, node.latLon.lat];
