...
var dispatch = d3.dispatch('authLoading', 'authDone', 'change', 'loading',
    'loaded'),
...
var parsers = {
    node: function nodeData(obj) {
        ...
        return new osmNode({
            ...
            action: attrs.action && attrs.action.value,
...
function parseMapBounds(xml) {
    if (!xml || !xml.childNodes) return;

    var bounds = xml.childNodes[0].getElementsByTagName('bounds');
    if (bounds.length === 1) {
        bounds = bounds[0];
        return {
...
function toDom(x) {
    var parsed = (new DOMParser()).parseFromString(x, 'text/xml');
    if (!parsed || !parsed.childNodes) {
        return { dom: parsed, err: 'empty file' };
    }
    /* In case of parsing errors DOMParser does not throw exceptions, but
     * returns an error document instead; it looks like this:
     * <parsererror ...>
     *   ...
     *   <div>"error on line 4 at column 21: ..."</div>
     * </parsererror>
     */
    var errEles = parsed.getElementsByTagName('parsererror');
    if (errEles.length > 0) {
        var divEles = errEles[0].getElementsByTagName('div');
        var errMsg = divEles.length > 0 && divEles[0].childNodes.length > 0
            ? divEles[0].childNodes[0].nodeValue
            : 'error when parsing the xml document';
        return { dom: null, err: errMsg };
    } else {
        return { dom: parsed, err: null };
    }
}
...
export default {
    ...
    initServerMetadata: function(serverMetadata) {
        urlroot = serverMetadata.url;
        oauth = osmAuth({
            url: urlroot,
            oauth_consumer_key: serverMetadata.oauth_consumer_key,
            oauth_secret: serverMetadata.oauth_secret,
            loading: authLoading,
            done: authDone
        });
        chunckSize = serverMetadata.chunk_size;
    },
    ...
    loadMultiple: function(ids, callback) {
        ...
        _.each(_.groupBy(_.uniq(ids), osmEntity.id.type), function(v, k) {
            ...
            _.each(_.chunk(osmIDs, chunckSize), function(arr) {
                that.loadFromAPI(
    ...
    putChangeset: function(changeset, changes, callback) {
        ...
        function uploadedChangeset(err) {
            ...
            // Still attempt to close changeset, but ignore response because
            // #2667
            oauth.xhr({
                method: 'PUT',
    ...
    userDetails: function(callback) {
        ...
        function done(err, user_details) {
            ...
            if (changesets && changesets[0] &&
                changesets[0].getAttribute('count')) {
    ...
    status: function(callback) {
        function done(xml) {
            ...
            for (var i = 0; i < elements.length; i++) {
                // needs unencode?
                var regex = elements[i].getAttribute('regex');
                if (regex) {
                    regexes.push(regex);
                }
            }
    ...
    loadInBounds: function(mapBounds, callback) {
        if (off) return;

        var extent = geoExtentFromBounds(mapBounds);

        this.loadFromAPI(
            '/api/0.6/map?bbox=' + extent.toParam(),
            function(err, parsed) {
                if (callback) {
                    callback(err, {
                      id: 'dummy_id',
    ...
    switch: function(options) {
        ...
        dispatch.call('change');
        this.reset();
        // eagerly load user details/changesets
    ...
    loadXMLFromFile: function(xmlFile, callback) {
        var reader = new FileReader();
        reader.onload = function(e) {
            var domRes = toDom(e.target.result);
            if (domRes.err) {
                callback(xmlFile, null, null, domRes.err);
            } else {
                var mapBounds = parseMapBounds(domRes.dom);
                var entities = parse(domRes.dom);
                callback(xmlFile, mapBounds, entities, domRes.err);
            }
        };
    ...
    loadXMLFromURL: function(xmlURL, callback) {
        d3.xml(xmlURL)
            .on("beforesend", function(req) { credentialXHR(req, xmlURL); })
            .get(function(err, dom) {
                if (!dom || err) {
                    callback(xmlURL, null, null, err);
                } else {
                    var mapBounds = parseMapBounds(dom),
                        entities = parse(dom);
                    callback(xmlURL, mapBounds, entities, err);
                }
            });
    }
