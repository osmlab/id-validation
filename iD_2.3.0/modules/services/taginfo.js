...
function filterValues(allowUpperCase) {
    return function(d) {
        ...
        if (d.value.match(/[;,]/) !== null) return false;
        // exclude uppercase letters
        if (!allowUpperCase && d.value.match(/[A-Z*]/) !== null) return false;
...
function filterRoles(geometry) {
    return function(d) {
        ...
        // exclude uppercase letters and some punctuation
        if (d.role.match(/[A-Z*;,]/) !== null) return false;
...
export default {
    ...
    init: function() {
        ...
        var params = { rp: 100, sortname: 'values_all', sortorder: 'desc',
            page: 1, debounce: false };
    ...
    keys: function(params, callback) {
        ...
        params = _.extend(
            { rp: 10, sortname: 'count_all', sortorder: 'desc', page: 1 },
            params
        );
    ...
    multikeys: function(params, callback) {
        ...
        params = clean(setSort(params));
        params = _.extend(
            { rp: 25, sortname: 'count_all', sortorder: 'desc', page: 1 },
            params
        );
    ...
    values: function(params, callback) {
        ...
        doRequest(url, params, false, callback, function(err, d) {
            if (err) {
                ...
                // In most cases we prefer taginfo value results with lowercase
                // letters. A few OSM keys expect values to contain uppercase
                // values (see #3377). This is not an exhaustive list
                // (e.g. `name` also has uppercase values) but these are the
                // fields where taginfo value lookup is most useful.
                var re =
                /network|taxon|genus|species|brand|grape_variety|_hours|_times/;
    ...
    roles: function(params, callback) {
        ...
        params = _.extend(
            {rp: 25, sortname: 'count_all_members', sortorder: 'desc', page: 1},
            params
        );
