...
export function svgAreas(projection, context) {
    ...
    function setPattern(d) {
        for (var i = 0; i < patternKeys.length; i++) {
            ...
            if (patterns.hasOwnProperty(d.tags[patternKeys[i]])) {
                this.style.fill = this.style.stroke = 'url("#pattern-' +
                    patterns[d.tags[patternKeys[i]]] + '")';
                return;
            }
        }
    ...
    return function drawAreas(selection, graph, entities, filter) {
        ...
        areas = d3.values(areas).filter(
            function hasPath(a) { return path(a.entity); });
        ...
        var clipPaths = context.surface().selectAll('defs')
            .selectAll('.clipPath')
            .filter(filter)
        ...
        paths = paths.enter()
            ...
            .each(function(entity) {
                ...
                this.setAttribute('class',
                    entity.type + ' area ' + layer + ' ' + entity.id);

                if (layer === 'fill') {
                    this.setAttribute('clip-path',
                        'url(#' + entity.id + '-clippath)');
