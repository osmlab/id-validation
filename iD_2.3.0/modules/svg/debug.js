...
export function svgDebug(projection, context) {
    ...
    function drawDebug(selection) {
        ...
        legendItems.enter()
            ...
            .attr(
                'class',
                function(d) { return 'debug-legend-item ' + d.class; }
        ...
        var extent = context.map().extent(),
            ...
            availableImagery = showsImagery &&
                multipolygons(dataImagery.filter(function(source) {
                    if (!source.polygon) return false;
                    return source.polygon.some(function(polygon) {
                        return
                            geoPolygonIntersectsPolygon(polygon, extent, true);
                    });
                }));
