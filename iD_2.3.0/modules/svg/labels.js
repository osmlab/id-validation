...
export function svgLabels(projection, context) {
    var path = d3.geoPath(projection),
        detected = utilDetect(),
        baselineHack =
            (detected.ie || detected.browser.toLowerCase() === 'edge'),
        rdrawn = rbush(),
        rskipped = rbush(),
        textWidthCache = {},
        entitybboxes = {};
    ...
    function drawAreaLabels(selection, entities, filter, classes, labels) {
        ...
        function hasText(d, i) {
            return labels[i].hasOwnProperty('x')
                && labels[i].hasOwnProperty('y');
        }
    }
    ...
    function drawLabels(selection, graph, entities, filter, dimensions,
        fullRedraw) {
        ...
        for (i = 0; i < entities.length; i++) {
            ...
            var preset = geometry === 'area' &&
                context.presets().match(entity, graph),
            ...
            for (k = 0; k < labelStack.length; k++) {
                ...
                if (geometry === matchGeom && hasVal &&
                    (matchVal === '*' || matchVal === hasVal)) {
        ...
        for (k = 0; k < labelable.length; k++) {
            ...
            for (i = 0; i < labelable[k].length; i++) {
                ...
                if (geometry === 'point') {
                    ...
                    // don't label vertices at low zoom because they don't have
                    // icons
                    p = getPointLabel(entity, width, fontSize, geometry);
        ...
        function getLineLabel(entity, width, height) {
            ...
            for (var i = 0; i < lineOffsets.length; i++) {
                ...
                // generate subpath and ignore paths that are invalid or don't
                // cross viewport.
                ...
                for (var j = 0; j < sub.length - 1; j++) {
                    ...
                    var b = sub[j + 1];
                    var num = Math.max(1,
                        Math.floor(geoEuclideanDistance(a, b) / boxsize / 2));
        ...
        function getAreaLabel(entity, width, height) {
            var centroid = path.centroid(entity.asGeoJSON(graph, true)),
                ...
                entitywidth = projection(extent[1])[0] -
                    projection(extent[0])[0];
        ...
        function tryInsert(bboxes, id, saveSkipped) {
            ...
            for (var i = 0; i < bboxes.length; i++) {
                ...
                if (bbox.minX < 0 || bbox.minY < 0 || bbox.maxX > dimensions[0]
                    || bbox.maxY > dimensions[1]) {
                    skipped = true;
                    break;
                }
    ...
    function filterLabels(selection) {
        ...
        if (mouse) {
            pad = 20;
            bbox = { minX: mouse[0] - pad, minY: mouse[1] - pad,
                maxX: mouse[0] + pad, maxY: mouse[1] + pad };
        ...
        for (var i = 0; i < selectedIDs.length; i++) {
            ...
            if (geometry === 'line') {
                ...
                pad = 10;
                bbox = { minX: point[0] - pad, minY: point[1] - pad,
                    maxX: point[0] + pad, maxY: point[1] + pad };
                ids.push.apply(ids, _.map(rdrawn.search(bbox), 'id'));
            }
        }
