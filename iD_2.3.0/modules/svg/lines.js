...
export function svgLines(projection, context) {
    ...
    function drawLines(selection, graph, entities, filter) {
        ...
        function drawLineGroup(selection, klass, isSelected) {
            ...
            // Optimization: call simple TagClasses only on enter selection.
            // This works because osmEntity.key is defined to include the entity
            // v attribute.
            lines.enter()
                .append('path')
                .attr('class', function(d) {
                    return 'way line ' + klass + ' ' + d.id +
                        (isSelected ? ' selected' : '') +
                        (oldMultiPolygonOuters[d.id]
                            ? ' old-multipolygon'
                            : ''
                        );
                })
                ...
                .attr('d', getPath)
                .call(
                    svgTagClasses(context).tags(svgRelationMemberTags(graph))
                );
        ...
        if (detected.ie) {
            oneways.each(
                function() { this.parentNode.insertBefore(this, this); }
            );
        }
    }
