...
export function svgVertices(projection, context) {
    ...
    function siblingAndChildVertices(ids, graph, extent) {
        ...
        function addChildVertices(entity) {
            if (!context.features().isHiddenFeature(
                    entity, graph, entity.geometry(graph))
                ) {
                var i;
                if (entity.type === 'way') {
    ...
    function draw(selection, vertices, klass, graph, zoom, siblings) {
        ...
        var enter = groups.enter()
            ...
            .attr(
                'class',
                function(d) { return 'node vertex ' + klass + ' ' + d.id; }
            )
            .call(svgTagClasses(context));
        ...
        groups
            ...
            .classed(
                'sibling',
                function(entity) { return entity.id in siblings; }
            ).classed(
                'shared',
                function(entity) { return graph.isShared(entity); }
            ).classed(
                'endpoint',
                function(entity) { return entity.isEndpoint(graph); }
            ).call(setAttributes);
    }


    function drawVertices(selection, graph, entities, filter, extent, zoom) {
        var siblings = siblingAndChildVertices(
                context.selectedIDs(), graph, extent),
    ...
    function drawHover(selection, graph, extent, zoom) {
        var hovered = hover
            ? siblingAndChildVertices([hover.id], graph, extent)
            : {};
        var layer = selection.selectAll('.layer-hit');
