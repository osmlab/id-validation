...
export function uiBackground(context) {
    ...
    function background(selection) {
            ...
            selection.each(function(d, i, nodes) {
                ...
                if (d === previous) {
                    item.call(tooltip()
                        ...
                        .html(true)
                        .title(function() {
                            var tip = '<div>' + t('background.switch') +
                                '</div>';
                            return uiTooltipHtml(tip, uiCmd('Ctrl+' + key));
                        })
                    );
        ...
        function clickSetGridsOpt(d) {
            d3.event.preventDefault();
            context.background().numGridSplits(d.numSplit);
            selectGridsOpt();
        }

        function selectGridsOpt() {
            function active(d) {
                return context.background().numGridSplits() === d.numSplit;
            }

            content.selectAll('.gridsopt, .custom_gridsopt')
                .classed('active', active)
                .selectAll('input')
                .property('checked', active);
        }
        ...
        function drawList(layerList, type, change, filter) {
            ...
            layerList.selectAll('li.layer')
                ...
                .style(
                    'display',
                    layerList.selectAll('li.layer').data().length > 0
                        ? 'block'
                        : 'none'
                );
        }

        function drawGridsOptionList() {
              var gridsOptions = gridsOptionList.selectAll('li.gridsopt')
                .data(
                    [
                        {numSplit: 2, name: '2 x 2 Grids'},
                        {numSplit: 3, name: '3 x 3 Grids'},
                        {numSplit: 4, name: '4 x 4 Grids'},
                        {numSplit: 5, name: '5 x 5 Grids'},
                        {numSplit: 6, name: '6 x 6 Grids'},
                        {numSplit: 0, name: 'No Grids'}
                    ],
                    function(d) { return d.name; }
                );

            var enter = gridsOptions.enter()
                .insert('li', '.custom_gridsopt')
                .attr('class', 'gridsopt');
            ...
            gridsOptionList.style('display', 'block');
        }


        function update() {
            backgroundList.call(drawList, 'radio', clickSetSource,
                function(d) { return !d.overlay; });
            overlayList.call(drawList, 'checkbox', clickSetOverlay,
                function(d) { return d.overlay; });
        ...
        function rotateBackgroundSource() {
            var sources = (context.background()
                    .sources(context.map().extent())
                    .filter(function(d) { return !d.overlay})
                    .sort(sortSources));
                curr = context.background().baseLayerSource(),
                currIdx = sources.indexOf(curr),
                next = sources[(currIdx + 1) % sources.length];
            if (next) {
                clickSetSource(next);
            }
        }
        ...
        var minimapLabel = controls
            ...
            .call(tooltip()
                .html(true)
                .title(uiTooltipHtml(
                    t('background.minimap.tooltip'),
                    t('background.minimap.key')
                )).placement('top')
            );
        ...
        var keybinding = d3keybinding('background')
            .on(key, toggle)
            .on(uiCmd('Ctrl+' + key), quickSwitch)
            .on(uiCmd('\u2318' + key), rotateBackgroundSource)  // Command+key
