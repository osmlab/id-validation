    ...
    function commit(selection) {
        if (!changeset) {
            var detected = utilDetect(),
                tags = {
                    created_by: ('iD-mystery-machine ' + context.version).substr(0, 255),
        ...
        context.connection().userDetails(function(err, user) {
            ...
            prose
                .html(t(
                    'commit.upload_explanation_with_user',
                    { user: userLink.html() }
                ));
        });
        ...
        function zoomToEntity(change) {
            ...
            if (change.changeType !== 'deleted' &&
                context.graph().entity(entity.id).geometry(context.graph())
                    !== 'vertex') {
        ...
        function change(onInput) {
            return function() {
                ...
                var expanded = !tagSection.selectAll(
                    'a.hide-toggle.expanded').empty();
