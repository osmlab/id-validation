...
export function uiEditMenu(context, operations) {
    ...
    var p = 8,               // top padding
        ...
        menuSideMargin = 10,
        tooltipWidth = 200,
        // a reasonable guess, real height depends on tooltip contents
        tooltipHeight = 200;
    ...
    var editMenu = function (selection) {
        ...
        if (!isRTL && (center[0] + menuSideMargin + menuWidth) >
                (viewport.width - vpSideMargin)) {
            // menu is going left-to-right and near right viewport edge, go left
            // instead
            isRTL = true;
        } else if (isRTL &&
            (center[0] - menuSideMargin - menuWidth) < vpSideMargin) {
            // menu is going right-to-left and near left viewport edge, go right
            // instead
            isRTL = false;
        }

        offset[0] =
            (isRTL ? -1 * (menuSideMargin + menuWidth) : menuSideMargin);

        if (center[1] + menuHeight > (viewport.height - vpBottomMargin)) {
            // menu is near bottom viewport edge, shift upwards
            offset[1] = -1 *
                (center[1] + menuHeight - viewport.height + vpBottomMargin);
        }
        ...
        var button = menu.selectAll('.edit-menu-item')
            ...
            .attr(
                'class',
                function (d) { return 'edit-menu-item edit-menu-item-' + d.id; }
        ...
        function mouseover(d, i) {
            ...
            if (tipX + tooltipWidth > viewport.right) {
                // tip is going left-to-right and near right viewport edge, go
                // left instead
                tipX = viewport.left + origin[0] - 4 - tooltipWidth;
            } else if (tipX < viewport.left) {
                // tip is going right-to-left and near left viewport edge, go
                // right instead
                tipX = viewport.left + origin[0] + menuSideMargin + menuWidth;
            }
