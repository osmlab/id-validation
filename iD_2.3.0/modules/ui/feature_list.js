    ...
    function featureList(selection) {
        ...
        function features() {
            ...
            if (idMatch) {
                result.push({
                    id: idMatch[0],
                    geometry: idMatch[1] === 'n'
                        ? 'point'
                        : idMatch[1] === 'w' ? 'line' : 'relation',
                    type: idMatch[1] === 'n'
                        ? t('inspector.node')
                        : idMatch[1] === 'w'
                              ? t('inspector.way')
            ...
            var locationMatch = sexagesimal.pair(q.toUpperCase())
                || q.match(/^(-?\d+\.?\d*)\s+(-?\d+\.?\d*)$/);

            if (locationMatch) {
                var loc = [
                    parseFloat(locationMatch[0]),
                    parseFloat(locationMatch[1])
                ];
                result.push({
                    id: -1,
                    geometry: 'point',
            ...
            var visible = context.surface().selectAll('.point, .line, .area')
                .nodes();
            ...
            (geocodeResults || []).forEach(function(d) {
                ...
                if (d.osm_type && d.osm_id) {
                    result.push({
                        ...
                        geometry: d.osm_type === 'relation'
                            ? 'relation'
                            : d.osm_type === 'way' ? 'line' : 'point',
                        type: d.type !== 'yes'
                            ? (d.type.charAt(0).toUpperCase() +
                                d.type.slice(1)).replace('_', ' ')
                            : (d.class.charAt(0).toUpperCase() +
                                d.class.slice(1)).replace('_', ' '),
                        name: d.display_name,
        ...
        function drawList() {
            ...
            var noResultsWorldwide = geocodeResults &&
            ...
            list.selectAll('.no-results-item')
                .style(
                    'display',
                    (value.length && !results.length) ? 'block' : 'none'
                );

            list.selectAll('.geocode-item')
                .style(
                    'display',
                    (value && geocodeResults === undefined) ? 'block' : 'none'
                );
        ...
        function mouseover(d) {
            ...
            context.surface().selectAll(
                utilEntityOrMemberSelector([d.id], context.graph())
            ).classed('hover', true);
        }
            ...
            if (d.location) {
                ...
                if (d.entity.type === 'node') {
                    ...
                    var center = context.projection(context.map().center()),
                        edge = geoChooseEdge(context.childNodes(d.entity),
                            center, context.projection);
