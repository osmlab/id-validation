...
export function uiFieldCheck(field, context) {
    ...
    function checkImpliedYes() {
        ...
        // where implied oneway tag exists (e.g. `junction=roundabout`)
        // #2220, #1841
        if (field.id === 'oneway') {
            var entity = context.entity(entityId);
            for (var key in entity.tags) {
                if (key in osmOneWayTags &&
                    (entity.tags[key] in osmOneWayTags[key])) {
                    impliedYes = true;
    ...
    function reverserSetText(selection) {
        ...
        var first = entity.first(),
            last = entity.isClosed()
                ? entity.nodes[entity.nodes.length - 2]
                : entity.last(),
    ...
    var check = function(selection) {
        ...
        if (field.type === 'onewayCheck') {
            enter
                ...
                .attr(
                    'class',
                    'reverser button' + (reverserHidden() ? ' hide' : '')
                ).attr('href', '#')
                .append('span')
