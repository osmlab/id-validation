...
export function uiFieldCombo(field, context) {
    ...
    function displayValue(tval) {
        ...
        if (optstrings) {
            var match = _.find(comboData,
                function(o) { return o.key === tval && o.value; });
            if (match) {
                return match.value;
            }
        }
    ...
    function setTaginfoValues(q, callback) {
        ...
        var hasCountryPrefix = isNetwork && country &&
            country.indexOf(q.toLowerCase()) === 0;
        if (hasCountryPrefix) {
            query = country + ':';
        }
    ...
    function setPlaceholder(d) {
        ...
        if (isMulti || isSemi) {
            ...
            var vals = _.map(d, 'value').filter(
                    function(s) { return s.length < 20; }
                ),
        ...
        if (isMulti || isSemi) {
            ...
            if (isMulti) {
                ...
                Object.keys(tags).forEach(function(key) {
                    if (key.indexOf(field.key) !== 0
                        || tags[key].toLowerCase() !== 'yes')
                        return;
                ...
                // Set keys for form-field modified (needed for undo and reset
                // buttons)..
