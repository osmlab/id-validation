...
export function uiFieldWikipedia(field, context) {
    ...
    function change(skipWikidata) {
        var value = utilGetSetValue(title),
            ...
            l = m && _.find(dataWikipedia,
                  function(d) { return m[1] === d[2]; }),
            syncTags = {};
        ...
        if (l) {
            ...
            if (m[3]) {
                ...
                try {
                    ...
                    anchor = decodeURIComponent(m[3].replace(
                        /\.([0-9A-F]{2})/g, '%$1'));
                } catch (e) {
        ...
        wikidata.itemsByTitle(language()[2], value, function(title, data) {
            ...
            var value = qids &&
                _.find(qids, function(id) { return id.match(/^Q\d+$/); });
        ...
        if (l) {
            ...
            if (anchor) {
                try {
                    ...
                    anchor = encodeURIComponent(anchor.replace(/ /g, '_'))
                        .replace(/%/g, '.');
                } catch (e) {
