...
import { uiGridFocus } from './gridfocus';
...
import { uiNextError } from './next_error';
import { uiPreviousError } from './previous_error';
import { uiRestore } from './restore';
import { uiSave } from './save';
import { uiSaveLocal } from './save_local';
...
export function uiInit(context) {
    ...
    function render(container) {
        ...
        limiter.append('div')
            .attr('class', 'prev-error-wrap col13')
            .call(uiPreviousError(context));

        limiter.append('div')
            .attr('class', 'next-error-wrap col13')
            .call(uiNextError(context));

        limiter.append('div')
            .attr('class', 'save-local-wrap col13')
            .call(uiSaveLocal(context));
        ...
        controls
            .append('div')
            .attr('class', 'map-control gridfocus-control')
            .call(uiGridFocus(context));
        ...
        var keybinding = d3keybinding('main')
            .on('\u232B', function() { d3.event.preventDefault(); }) //backspace
            .on('\u2190', pan([pa, 0]))  // left arrow
            .on('\u2191', pan([0, pa]))  // up arrow
            .on('\u2192', pan([-pa, 0])) // right arrow
            .on('\u2193', pan([0, -pa]))  // down arrow
            // shift + left/up/right/down arrow for the following
            .on(
                ['\u21E7\u2190', uiCmd('\u2318\u2190')],
                pan([mapDimensions[0], 0])
            ).on(
                ['\u21E7\u2191', uiCmd('\u2318\u2191')],
                pan([0, mapDimensions[1]])
            ).on(
                ['\u21E7\u2192', uiCmd('\u2318\u2192')],
                pan([-mapDimensions[0], 0])
            ).on(
        ...
        if (!uiInitCounter++) {
            if (!hash.startWalkthrough) {
                context.container().call(uiSplash(context));
                if (!context.restoreDisabled()) {
