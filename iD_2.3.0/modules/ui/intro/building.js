...
export function uiIntroBuilding(context, reveal) {
    ...
    function addHouse() {
        ...
        timeout(function() {
            var tooltip = reveal(
                    'button.add-area',
                    t(
                        'intro.buildings.add_building',
                        { button: icon('#icon-area', 'pre-text') }
                    )
                );
    ...
    function startHouse() {
        ...
        timeout(function() {
            ...
            context.map().on('move.intro drawn.intro', function() {
                revealHouse(house,
                    t('intro.buildings.start_building'), { duration: 0 });
            });
    ...
    function continueHouse() {
        ...
        context.map().on('move.intro drawn.intro', function() {
            revealHouse(house,
                t('intro.buildings.continue_building'), { duration: 0 });
        });
        ...
        context.on('enter.intro', function(mode) {
            if (mode.id === 'draw-area') {
                ...
                var graph = context.graph(),
                    ...
                    points = _.uniq(nodes).map(
                        function(n) { return context.projection(n.loc); });
    ...
    function retryHouse() {
        ...
        context.map().on('move.intro drawn.intro', function() {
            revealHouse(
                house,
                t('intro.buildings.retry_building'),
                {
                    duration: 0,
                    buttonText:
                    t('intro.ok'),
    ...
    function chooseCategoryBuilding() {
        ...
        if (context.mode().id !== 'select' || !ids.length ||
            ids[0] !== houseId) {
        ...
        timeout(function() {
            ...
            var button = d3.select(
                    '.preset-category-building .preset-list-button');

            reveal(
                button.node(),
                t(
                    'intro.buildings.choose_category_building',
    ...
    function choosePresetHouse() {
        ...
        timeout(function() {
            ...
            var button = d3.select(
                    '.preset-building-house .preset-list-button');

            reveal(
                button.node(),
                t(
                    'intro.buildings.choose_preset_house',
    ...
    function closeEditorHouse() {
        ...
        timeout(function() {
            reveal(
                '.entity-editor-pane',
                t(
                    'intro.buildings.close',
                    { button: icon('#icon-apply', 'pre-text') }
                )
            );
        }, 500);
    ...
    function rightClickHouse() {
        ...
        context.map().on('move.intro drawn.intro', function() {
            revealHouse(house,
                t('intro.buildings.rightclick_building'), { duration: 0 });
        });
    ...
    function clickSquare() {
        ...
        revealEditMenu(
            menuCoords,
            t(
                'intro.buildings.square_building',
                { button: icon('#operation-orthogonalize', 'pre-text') }
            )
        );
        ...
        context.history().on('change.intro', function() {
            ...
            // Something changed.  Wait for transition to complete and check
            // undo annotation.
            timeout(function() {
                if (context.history().undoAnnotation() ===
    ...
    function startTank() {
        ...
        timeout(function() {
            ...
            context.map().on('move.intro drawn.intro', function() {
                revealTank(tank,
                    t('intro.buildings.start_tank'), { duration: 0 });
            });
    ...
    function continueTank() {
        ...
        context.map().on('move.intro drawn.intro', function() {
            revealTank(tank,
                t('intro.buildings.continue_tank'), { duration: 0 });
        });
    ...
    function searchPresetTank() {
        ...
        function continueTo(nextStep) {
            ...
            context.on('enter.intro', null);
            context.history().on('change.intro', null);
            d3.select('.preset-search-input').on(
                'keydown.intro keyup.intro', null);
            nextStep();
        }
    }
    ...
    function rightClickTank() {
        ...
        timeout(function() {
            ...
            context.map().on('move.intro drawn.intro', function() {
                revealTank(tank,
                    t('intro.buildings.rightclick_tank'), { duration: 0 });
            });
    ...
    function clickCircle() {
        ...
        revealEditMenu(
            menuCoords,
            t(
                'intro.buildings.circle_tank',
                { button: icon('#operation-circularize', 'pre-text') }
            )
        );
        ...
        context.history().on('change.intro', function() {
            ...
            timeout(function() {
                if (context.history().undoAnnotation() ===
                    t('operations.circularize.annotation.area')) {
                    continueTo(play);
                } else {
                    continueTo(retryClickCircle);
                }
    ...
    function play() {
        ...
        reveal(
            '#id-container',
            t(
                'intro.buildings.play',
                { next: t('intro.startediting.title') }
            ),
            {
