...
function checkKey(key, text) {
    if (t(key, { default: undefined}) === undefined) {
        ...
        if (typeof console !== 'undefined')
            console.log(missing); // eslint-disable-line
    }
}
...
export function localize(obj) {
    ...
    if (street) {
        ...
        var addrTags = [
            'block_number', 'city', 'county', 'district', 'hamlet',
            'neighbourhood', 'postcode', 'province', 'quarter', 'state',
            'subdistrict', 'suburb'
        ];
...
// Used to detect squareness.. some duplicataion of code from
// actionOrthogonalize.
export function isMostlySquare(points) {
    // note: uses 15 here instead of the 12 from actionOrthogonalize because
    // actionOrthogonalize can actually straighten some larger angles as it
    // iterates
