...
export function uiIntro(context) {
    ...
    function intro(selection) {
        ...
        context.history().merge(
            d3.values(coreGraph(context).load(introGraph).entities));
        context.history().checkpoint('initial');
        context.background().bing();
        overlays.forEach(
            function (d) { context.background().toggleOverlayLayer(d); });
        ...
        var buttons = buttonwrap
            ...
            .attr(
                'class',
                function(d, i) { return 'chapter chapter-' + chapterFlow[i]; }
