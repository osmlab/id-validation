...
export function uiIntroLine(context, reveal) {
    ...
    function addLine() {
        ...
        timeout(function() {
            var tooltip = reveal(
                    'button.add-line',
                    t(
                        'intro.lines.add_line',
                        { button: icon('#icon-line', 'pre-text') }
                    )
                );
    ...
    function drawLine() {
        ...
        timeout(function() {
            ...
            reveal(
                box,
                t(
                    'intro.lines.intersect',
                    { name: t('intro.graph.name.flower-street') }
                )
            );
    ...
    function chooseCategoryRoad() {
        ...
        timeout(function() {
            ...
            reveal(
                button.node(),
                t(
                    'intro.lines.choose_category_road',
    ...
    function choosePresetResidential() {
        ...
        subgrid.selectAll(
            ':not(.preset-highway-residential) .preset-list-button'
        ...
        timeout(function() {
            reveal(
                subgrid.node(),
                t(
                    'intro.lines.choose_preset_residential',
                    { preset: residentialPreset.name() }
                ),
                {
                    tooltipBox: '.preset-highway-residential ' +
                        '.preset-list-button',
                    duration: 300
                }
            );
    ...
    function retryPresetResidential() {
        ...
        timeout(function() {
            ...
            reveal(
                button.node(),
                t(
                    'intro.lines.retry_preset_residential',
                    { preset: residentialPreset.name() }
                )
            );
    ...
    function updateLine() {
        ...
        if (!context.hasEntity(woodRoadId) ||
            !context.hasEntity(woodRoadEndId)) {
            return chapter.restart();
        }
    ...
    function continueDragMidpoint() {
        ...
        context.map().on('move.intro drawn.intro', function() {
            ...
            reveal(
                box,
                t('intro.lines.continue_drag_midpoint'),
                {
                    duration: 0,
                    buttonText: t('intro.ok'),
                    buttonCallback: advance
    ...
    function deleteLines() {
        ...
        timeout(function() {
            ...
            reveal(
                box,
                t(
                    'intro.lines.delete_lines',
                    { street: t('intro.graph.name.12th-avenue') }
                ),
                { buttonText: t('intro.ok'), buttonCallback: advance }
            );
            ...
            context.history().on('change.intro', function() {
                // after any transition (e.g. if user deleted intersection)
                timeout(function() {
                    continueTo(deleteLines);
                }, 500);
            });
    ...
    function rightClickIntersection() {
        ...
        timeout(function() {
            ...
            reveal(
                box,
                t(
                    'intro.lines.rightclick_intersection',
                    {
    ...
    function splitIntersection() {
        ...
        revealEditMenu(
            ...
            t(
                ...
                {
                    button: icon('#operation-split', 'pre-text'),
        ...
        context.history().on('change.intro', function(changed) {
            ...
            timeout(function() {
                if (context.history().undoAnnotation() ===
                    t('operations.split.annotation.line')) {
    ...
    function didSplit() {
        ...
        var ids = context.selectedIDs();
        var string = 'intro.lines.did_split_' +
            (ids.length > 1 ? 'multi' : 'single');
        ...
        timeout(function() {
            ...
            reveal(
                ...
                t(
                    ...
                    { selected: selected, other1: other, other2: other }
                )
            );
    ...
    function multiDelete() {
        ...
        revealEditMenu(
            menuCoords,
            t(
                'intro.lines.multi_delete',
                { button: icon('#operation-delete', 'pre-text') }
            )
        );
        ...
        context.on('exit.intro', function() {
            if (context.hasEntity(washingtonSegmentId) ||
                context.hasEntity(twelfthAvenueId)) {
                // left select mode but roads still exist
                return continueTo(multiSelect);
            }
        });
        ...
        context.history().on('change.intro', function() {
            if (context.hasEntity(washingtonSegmentId) ||
                context.hasEntity(twelfthAvenueId)) {
                // changed something but roads still exist
