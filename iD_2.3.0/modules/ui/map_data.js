...
export function uiMapData(context) {
    var key = t('map_data.key'),
        ...
        fills = ['none', 'wireframe', 'partial', 'full'],
    ...
    function map_data(selection) {
        ...
        function drawMapillaryItems(selection) {
            var mapillaryImages = layers.layer('mapillary-images'),
                ...
                supportsMapillaryImages = mapillaryImages &&
                    mapillaryImages.supported(),
                supportsMapillarySigns = mapillarySigns &&
                    mapillarySigns.supported(),
                showsMapillaryImages = supportsMapillaryImages &&
                    mapillaryImages.enabled(),
                showsMapillarySigns = supportsMapillarySigns &&
        ...
        function drawOsmItem(selection) {
            ...
            // Enter
            var enter = osmLayerItem.enter()
                .append('ul')
                .attr('class', 'layer-list layer-list-osm')
                .append('li')
                .classed('list-item-osm', true);

            enter
                .append('button')
                .attr('class', 'list-item-osm-url')
                .call(tooltip()
                    .title(t('osm_file.load_url'))
                    .placement((textDirection === 'rtl') ? 'right' : 'left'))
                .on('click', function() {
                    var osmURL = window.prompt(t('osm_file.prompt_url'), '');
                    if (osmURL) {
                        hidePanel();
                        context.showOSMLoadingMsg(t('osm_file.loading'));
                        services.osm.loadXMLFromURL(
                            osmURL,
            ...
            enter
                .append('button')
                .attr('class', 'list-item-osm-file')
                .call(tooltip()
                    .title(t('osm_file.load_file'))
                    .placement((textDirection === 'rtl') ? 'right' : 'left'))
                .on('click', function() {
                    d3.select(document.createElement('input'))
                        .attr('type', 'file')
                        .on('change', function() {
                            if (d3.event.target.files.length === 1) {
                                hidePanel();
                                context.showOSMLoadingMsg(
                                    t('osm_file.loading'));
                                services.osm.loadXMLFromFile(
                                    d3.event.target.files[0],
                                    context.osmXMLLoadedFromFile
                                );
                            }
                        })
                        .node().click();
            ...
            labelOsm
                .append('span')
                .text(t('osm_file.title'));

            // Update
            osmLayerItem = osmLayerItem
                .merge(enter);
            ...
            osmLayerItem
                ...
                .classed('deemphasize', context.osmXMLEntities() === null);

            osmLayerItem
                .selectAll('input')
                .property('checked', context.osmXMLEntities() !== null);

            osmLayerItem
                .selectAll('button')
                .property('disabled', context.osmXMLEntities() !== null);
        }
        ...
        function drawList(selection, data, type, name, change, active) {
            ...
            label
                .append('span')
                .text(
                    function(d) { return t(name + '.' + d + '.description'); }
                );
        ...
        function update() {
            ...
            if (context.editInBoundsMode() ===
                ...
                dataLayerContainer.call(drawOsmItem);
            }

            fillList.call(drawList, fills, 'radio', 'area_fill', setFill,
                showsFill);
            featureList.call(drawList, features, 'checkbox', 'feature',
                clickFeature, showsFeature);
        }
        ...
        function toggleFillNone() {
            d3.event.preventDefault();
            setFill(fillSelected === 'none' ? fillDefault : 'none');
        }
        ...
        function toggleHighlightEdited() {
            ...
            surface.classed('highlight-edited',
                !surface.classed('highlight-edited'));
        }
        ...
        var keybinding = d3keybinding('features')
            ...
            .on(t('area_fill.none.key'), toggleFillNone)
            .on(t('highlight_edited.key'), toggleHighlightEdited)
