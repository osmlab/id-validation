...
export function uiPanelBackground(context) {
    ...
    function getVintage(selection) {
        // tile near viewport center
        var tile = d3.select('.layer-background img.tile-center');
        if (tile.empty()) return;

        var d = tile.datum(),
            zoom = (d && d.length >= 3 && d[2]) ||
                Math.floor(context.map().zoom()),
        ...
        background.baseLayerSource().getVintage(
            center,
            d,
            function(err, result) {
              currVintage = (result && result.range) ||
