...
export function uiPanelMeasurement(context) {
    ...
    function redraw(selection) {
        var resolver = context.graph(),
            selected = _.filter(context.selectedIDs(),
                function(e) { return context.hasEntity(e); }),
        ...
        selection
            ...
            .text(singular ||
                t('info_panels.measurement.selected', { n: selected.length }));
        ...
        if (!singular) {
            list
                ...
                .text(t('info_panels.measurement.center') + ': ' +
                    center[1].toFixed(OSM_PRECISION) + ', ' +
                    center[0].toFixed(OSM_PRECISION)
                );
            return;
        }
        ...
        if (geometry === 'line' || geometry === 'area') {
            var closed = (entity.type === 'relation') ||
                    (entity.isClosed() && !entity.isDegenerate()),
                feature = entity.asGeoJSON(resolver),
                length = radiansToMeters(d3GeoLength(toLineString(feature))),
                lengthLabel = t('info_panels.measurement.' +
            ...
            list
                ...
                .text(t('info_panels.measurement.geometry') + ': ' +
                    (closed ? t('info_panels.measurement.closed') + ' ' : '') +
                    t('geometry.' + geometry) );
            ...
            list
                ...
                .text(t('info_panels.measurement.centroid') + ': ' +
                    centroid[1].toFixed(OSM_PRECISION) + ', ' +
                    centroid[0].toFixed(OSM_PRECISION)
                );
            ...
            var centerLabel = t('info_panels.measurement.' +
                    (entity.type === 'node' ? 'location' : 'center'));
