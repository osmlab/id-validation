...
export function uiPreset(context) {
    ...
    function UIField(field, entity, show) {
        ...
        field.shown = function() {
            return field.id === 'name' || field.show ||
                _.some(field.keys, function(key) { return !!tags[key]; });
        };
    ...
    function content(selection) {
        if (!fieldsArr) {
            ...
            if (entity.isHighwayIntersection(context.graph()) &&
                presets.field('restrictions')) {
                fieldsArr.push(
                    UIField(presets.field('restrictions'), entity, true));
            }
        ...
        var shown = fieldsArr.filter(function(field) { return field.shown(); }),
            notShown = fieldsArr.filter(
                function(field) { return !field.shown(); });
        ...
        fields
            ...
            .each(function(field) {
                ...
                var reference = uiTagReference(
                        field.reference || { key: referenceKey }, context);
                ...
                d3.select(this)
                    ...
                    .selectAll('input')
                    .on('keydown', function() {
                        // if user presses enter, and combobox is not active,
                        // accept edits..
                        if (d3.event.keyCode === 13 &&
                            d3.select('.combobox').empty()) {
        ...
        input
            ...
            .attr('placeholder', function() {
                ...
                return placeholder.slice(0,3).join(', ') +
