...
export function uiPresetList(context) {
    ...
    function presetList(selection) {
        ...
        if (context.entity(id).isUsed(context.graph())) {
            messagewrap
                ...
                .on(
                    'click',
                    function() { dispatch.call('choose', this, currentPreset); }
                ).call(svgIcon(
                    (textDirection === 'rtl')
                        ? '#icon-backward'
                        : '#icon-forward'
        ...
        function keypress() {
            ...
            if (d3.event.keyCode === 13 && value.length) {
                list.selectAll('.preset-list-item:first-child').datum()
                    .choose();
            }
        }
    ...
    function drawList(list, presets) {
        ...
        items.enter()
            ...
            .attr('class', function(item) {
                return 'preset-list-item preset-' +
                    item.preset.id.replace('/', '-');
        ...
        item.choose = function() {
            ...
            if (shown) {
                ...
                box.transition()
                    ...
                    .style('opacity', '1')
                    .style('max-height',
                        200 + preset.members.collection.length * 80 + 'px')
    ...
    function PresetItem(preset) {
        ...
        item.preset = preset;
        item.reference = uiTagReference(
            preset.reference(context.geometry(id)), context);
    ...
    presetList.entityID = function(_) {
        ...
        id = _;
        presetList.preset(
            context.presets().match(context.entity(id), context.graph()));
        return presetList;
    };
