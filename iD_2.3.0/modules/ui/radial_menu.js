...
export function uiRadialMenu(context, operations) {
    ...
    var radialMenu = function(selection) {
        ...
        menu
            ...
            .attr(
                'd',
                // Force positive-length path (#1305)
                'M' + r * Math.sin(a0) + ',' + r * Math.cos(a0) + ' A' + r +
                    ',' + r + ' 0 ' + (operations.length > 5 ? '1' : '0') +
                    ',0 ' + (r * Math.sin(a1) + 1e-3) + ',' +
                    (r * Math.cos(a1) + 1e-3)
        ...
        function mouseover(d, i) {
            var rect = context.surfaceRect(),
                ...
                left = rect.left + (r + 25) * Math.sin(angle) + center[0] +
                    'px',
                bottom = rect.height - (r + 25) * Math.cos(angle) - center[1] +
                    'px',
                right = rect.width - (r + 25) * Math.sin(angle) - center[0] +
                    'px';
