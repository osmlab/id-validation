...
export function uiRawMembershipEditor(context) {
    ...
    function changeRole(d) {
        ...
        context.perform(
            actionChangeMember(
                d.relation.id,
                _.extend({}, d.member, { role: role }), d.index
            ),
    ...
    function addMembership(d, role) {
        ...
        if (d.relation) {
            context.perform(
                actionAddMember(
                    d.relation.id,
                    { id: id, type: context.entity(id).type, role: role }
                ),
    ...
    function relations(q) {
        ...
        context.intersects(context.extent()).forEach(function(entity) {
            ...
            var matched = context.presets().match(entity, graph),
                presetName = (matched && matched.name()) ||
    ...
    function rawMembershipEditor(selection) {
        ...
        parents.slice(0, 1000).forEach(function(relation) {
            relation.members.forEach(function(member, index) {
                if (member.id === entity.id) {
                    memberships.push(
                        { relation: relation, member: member, index: index });
                }
            });
        });
        ...
        selection.call(uiDisclosure()
            .title(t('inspector.all_relations') + ' (' + gt +
                memberships.length + ')')
            .expanded(true)
            .on('toggled', toggled)
            .content(content)
        ...
        function content(wrap) {
            ...
            label
                ...
                .text(function(d) {
                    var matched = context.presets().match(
                            d.relation, context.graph());
            ...
            newrow.selectAll('.member-entity-input')
                .call(d3combobox()
                    ...
                    .minItems(1)
                    .fetcher(
                        function(value, callback) {callback(relations(value));}
                    ).on('accept', onAccept)
                );
            ...
            function onAccept(d) {
                var role = list.selectAll('.member-row-new .member-role')
                        .property('value');
            ...
            function bindTypeahead(d) {
                ...
                function sort(value, data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        if (data[i].value.substring(0, value.length) ===
                            value) {
