    ...
    function content(wrap) {
        ...
        items
            .each(function(tag) {
                var row = d3.select(this),
                    // propagate bound data to child
                    key = row.select('input.key'),
                    value = row.select('input.value');
                ...
                if (isRelation && tag.key === 'type') {
                    ...
                    reference = uiTagReference(
                        { key: tag.key, value: tag.value }, context);
                }
        ...
        function valueChange(d) {
            ...
            tag[d.key] = this.value;
             ...
             // not a new row anymore
            if (newRow === d.key && d.key !== '' && d.value !== '') {
                newRow = undefined;
            }
