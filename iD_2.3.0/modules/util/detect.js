...
export function utilDetect(force) {
    ...
    detected.opera = (detected.browser.toLowerCase() === 'opera' &&
        parseFloat(detected.version) < 15 );

    detected.locale = (navigator.language || navigator.userLanguage || 'en-US');
    detected.language = detected.locale.split('-')[0];

    // Search `navigator.languages` for a better locale.. Prefer the first
    // language,
    // unless the second language is a culture-specific version of the first
    // one, see #3842
    ...
    // return that instead (except in the situation where 'en' might override
    // 'en-US')
