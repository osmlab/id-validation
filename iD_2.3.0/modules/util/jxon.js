export var JXON = new (function () {
  ...
  var
    sValueProp = 'keyValue', sAttributesProp = 'keyAttributes', sAttrPref = '@',
  ...
  function createObjTree (oParentNode, nVerb, bFreeze, bNesteAttr) {
    ...
    var
      ...
      vResult = bHighVerb ? {} : true;

    if (bChildren) {
      for (var oNode, nItem = 0; nItem < oParentNode.childNodes.length;
          nItem++) {
    ...
    if (bAttributes) {
      var
        ...
        sAPrefix = bNesteAttr
          ? ''
          : sAttrPref, oAttrParent = bNesteAttr ? {} : vResult;

      for (var oAttrib, nAttrib = 0; nAttrib < nAttrLen; nLength++, nAttrib++) {
        oAttrib = oParentNode.attributes.item(nAttrib);
        oAttrParent[sAPrefix + oAttrib.name.toLowerCase()] =
  ...
  this.build = function (
    ...
    var _nVerb = arguments.length > 1 && typeof nVerbosity === 'number'
      ? nVerbosity & 3
      : 1; /* put here the default verbosity level: */
...
// try: alert((new XMLSerializer()).serializeToString(newDoc));
