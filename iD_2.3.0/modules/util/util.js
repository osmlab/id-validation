...
export function utilGetAllNodes(ids, graph) {
    ...
    function getNodes(id) {
        ...
        if (entity.type === 'node') {
            ...
            entity.members.map(function(member) { return member.id; }).forEach(
                getNodes);
        }
    }
}
...
export function utilDisplayName(entity) {
    var localizedNameKey = 'name:' +
            utilDetect().locale.toLowerCase().split('-')[0],
...
export function utilQsString(obj, noencode) {
    function softEncode(s) {
      // encode everything except special characters used in certain hash
      // parameters: "/" in map states, ":", ",", {" and "}" in background
      return encodeURIComponent(s).replace(
          /(%2F|%3A|%2C|%7B|%7D)/g, decodeURIComponent);
    }
...
export function utilSetTransform(el, x, y, scale) {
    var prop = transformProperty = transformProperty ||
...
// first converts the strings to lowercase and replaces diacritic marks with
// ascii equivalents.
...
export var utilGetPrototypeOf = Object.getPrototypeOf ||
 ...
 * @returns {Function} a function that returns that value or the value if it's
 *          a function
 */
...
// Object.keys(obj).length or _.size(obj) only checks obj's own properties. But
// for things like 'entities' in graph (check constructor of coreGraph in
// graph.js), we need to count in the properties from obj's prototype to get
// how many exact entities are in the graph. So adding this function to help.
export function objectSizeWithProto(obj) {
    var size = 0;
    for (key in obj) size++;
    return size;
}
...
// Similar to above, but for checking whether obj is empty we can return early.
export function isObjectEmptyWithProto(obj) {
