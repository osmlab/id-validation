...
;(function(window, undefined) {
  ...
  (function() {
     ...
     * @type Boolean
     */
    support.timeout = isHostType(window, 'setTimeout') &&
    ...
    try {
      var o = {};
      support.descriptors = (setDescriptor(o, o, o),
          'value' in getDescriptor(o, o));
    } catch(e) {
    ...
    /**
     * Detect if own properties are iterated before inherited properties
     * (all but IE < 9).
     *
   ...
   * is buggy regardless of mode in IE < 9 and buggy in compatibility mode in
   * IE 9.
   * In Opera < 9.50 and some older/beta Mobile Safari versions using
   * `unshift()` generically to augment the `arguments` object will pave the
   * value at index 0 without incrimenting the other values's indexes.
   ...
   * @param {Number} deleteCount The number of elements to delete from the
   *        insert point.
  ...
  function createFunction() {
    ...
    createFunction = function(args, body) {
      ...
      runScript((freeDefine ? 'define.amd.' : 'Benchmark.') + prop +
          '=function(' + args + '){' + body + '}');
      result = anchor[prop];
      delete anchor[prop];
      return result;
    ...
    createFunction = support.browser &&
        (createFunction('', 'return"' + uid + '"') || noop)() == uid
            ? createFunction
            : Function;
  ...
  /**
   * Delay the execution of a function based on the benchmark's `delay`
   * property.
   * @private
  ...
  function forProps() {
    var forShadowed,
        skipSeen,
        forArgs = true,
        shadowed = ['constructor', 'hasOwnProperty', 'isPrototypeOf',
    ...
    (function(enumFlag, key) {
      ...
      skipSeen = enumFlag == 2;
      // IE < 9 incorrectly makes an object's properties non-enumerable if they
      // have the same name as other non-enumerable properties in its prototype
      // chain.
    ...
    forProps = function(object, callback, options) {
      ...
      if (allFlag && support.getAllKeys) {
        for (index = 0, keys = getAllKeys(object), length = keys.length;
            index < length; index++) {
          key = keys[index];
      ...
      // else iterate only enumerable properties
        ...
        for (key in object) {
          ...
          // incorrectly set a function's `prototype` property [[Enumerable]]
          // value to `true`. Because of this we standardize on skipping the
          // `prototype` property of functions regardless of their
          // [[Enumerable]] value.
          if ((done =
        ...
        if (!done &&
            (forArgs && isArguments(object) ||
                ((noCharByIndex || noCharByOwnIndex) &&
                  isClassOf(object, 'String') &&
            ...
            while (++index < length) {
                if ((done = callback(
                      iteratee[index], String(index), object) === false)
                ) {
                    break;
                }
            }
   ...
   * @private
   * @param {Function} fn The function.
   * @param {String} altSource A string used when a function's source code is
   *        unretrievable.
   ...
   * @returns {Boolean} Returns `true` if the value is of the specified class,
   *          else `false`.
   */
   ...
   * @returns {Boolean} Returns `true` if the property value is a non-primitive,
   *          else `false`.
   */
  ...
  function isPlainObject(value) {
    ...
    // Also check that the constructor is `Object` (i.e. `Object instanceof
    // Object`)
    var ctor = value.constructor;
    if ((support.nodeClass || !(typeof value.toString != 'function' &&
        typeof (value + '') == 'string')) &&
      ...
      // first iterated property is an object's own property then there are no
  ...
  function runScript(code) {
    ...
    // Firefox 2.0.0.2 cannot use script injection as intended because it
    // executes asynchronously, but that's OK because script injection is only
    // used to avoid the previously commented JaegerMonkey bug.
    try {
      // remove the inserted script *before* running the code to avoid
      // differences in the expected script element count/order of the document.
  ...
  function resolve() {
    ...
    if (bench.aborted) {
      // cycle() -> clone cycle/complete event -> compute()'s invoked
      // bench.run() cycle/complete
      me.teardown();
      clone.running = false;
      cycle(me);
    }
  ...
  function deepClone(value) {
    ...
    function forPropsCallback(subValue, subKey) {
      ...
      if (subValue === Object(subValue)) {
        queue[queue.length++] = { 'key': subKey, 'parent': clone,
            'source': value };
      }
      // assign non-objects
      else {
    ...
    do {
      ...
      if (value === Object(value)) {
        ...
        if (clone && clone != value &&
          ...
          if ((extensible = isExtensible(value))) {
            ...
            for (subIndex = 0, length = unmarked.length; subIndex < length;
                subIndex++) {
          ...
          if (!circular) {
            // mark object to allow quickly detecting circular references and
            // tie it to its clone
            if (extensible) {
      ...
      if (parent) {
        ...
        if (accessor || (descriptor && !(descriptor.configurable &&
  ...
  function each(object, callback, thisArg) {
    ...
    var fn = callback,
        index = -1,
        length = object.length,
        isSnapshot = !!(object.snapshotItem &&
            (length = object.snapshotLength)),
        isSplittable = (noCharByIndex || noCharByOwnIndex) &&
            isClassOf(object, 'String'),
  ...
  /**
   * Copies enumerable properties from the source(s) object to the destination
   * object.
   *
   * @static
   * @memberOf Benchmark
  ...
  function extend(destination, source) {
    // Chrome < 14 incorrectly sets `destination` to `undefined` when we
    // `delete arguments[0]` http://code.google.com/p/v8/issues/detail?id=839
  ...
  function filter(array, callback, thisArg) {
    ...
    if (callback == 'successful') {
      // callback to exclude those that are errored, unrun, or have hz of
      // Infinity
      callback = function(bench) { return bench.cycles && isFinite(bench.hz); };
    }
    else if (callback == 'fastest' || callback == 'slowest') {
      // get successful, sort by period + margin of error, and filter
      // fastest/slowest
  ...
  function hasKey() {
    ...
    hasKey = function(object, key) {
      ...
      return !!parent && key in Object(object) &&
    ...
    // for Safari 2
      ...
      hasKey = function(object, key) {
        ...
        if (object != null) {
          object = Object(object);
          object.__proto__ = [object.__proto__, object.__proto__ = null,
              result = key in object][0];
        }
        return result;
      };
    }
  ...
  /**
   * Modify a string by replacing named tokens with matching object property
   * values.
   * @static
   * @memberOf Benchmark
  ...
  function interpolate(string, object) {
    forOwn(object, function(value, key) {
      ...
      string = string.replace(RegExp('#\\{' + key.replace(
          /([.*+?^=!:${}()|[\]\/\\])/g, '\\$1') + '\\}', 'g'), value);
    });
    return string;
  }
   ...
   * @param {String|Object} name The name of the method to invoke OR options
   *        object.
   ...
   *
   * // invoke `run(true)`, treat benchmarks as a queue, and register invoke
   * callbacks
   ...
   *   'args': true,
   *
   *   // treat as queue, removing benchmarks from front of `benches` until
   *   empty
   *   'queued': true,
   *
  ...
  function invoke(benches, name) {
    ...
    /**
     * Invokes the method of the current object and if synchronous, fetches the
     * next.
     */
    function execute() {
      var listeners,
      ...
      // execute method
      result[index] = isClassOf(bench && bench[name], 'Function')
    ...
    function getNext(event) {
      ...
      // the "complete" listeners because they were already called above and
      // when used as part of `getNext` the `return false` will exit the
    ...
    function isAsync(object) {
      // avoid using `instanceof` here because of IE memory leak issues with
      // host objects
      var async = args[0] && args[0].async;
      return Object(object).constructor == Benchmark && name == 'run' &&
        ((async == null ? object.options.async : async) &&
            support.timeout || object.defer);
    }
    ...
    function raiseIndex() {
      ...
      if (queued) {
        // if queued remove the previous bench and subsequent skipped
        // non-entries
        do {
    ...
    if (isClassOf(name, 'String')) {
      ...
      name = options.name;
      args = isClassOf(args = 'args' in options ? options.args : [], 'Array')
          ? args
          : [args];
   ...
   * @param {String} [separator2=': '] The separator used between keys and value
  ...
  function emit(type) {
    ...
    if (events && (listeners = hasKey(events, event.type) &&
        events[event.type])) {
   ...
   * @returns {Number} Returns `-1` if slower, `1` if faster, and `0` if
   *          indeterminate.
   */
  ...
  function reset() {
    ...
    if (me.running && !calledBy.abort) {
      ...
      data = { 'destination': me, 'source': extend({},
          me.constructor.prototype, me.options) };
      do {
        forOwn(data.source, function(value, key) {
          ...
          if (value && typeof value == 'object') {
            ...
            if (changed) {
              changes[changes.length++] = { 'destination': destination,
                  'key': key, 'value': currValue };
            }
            queue[queue.length++] = { 'destination': currValue,
                'source': value };
          }
          // register a changed primitive
          else if (value !== currValue && !(value == null ||
            isClassOf(value, 'Function'))) {
            changes[changes.length++] = { 'destination': destination,
                'key': key, 'value': value };
          }
        });
      ...
      // if changed emit the `reset` event and if it isn't cancelled reset the
      // benchmark
      if (changes.length &&
          (me.emit(event = Event('reset')), !event.cancelled)) {
  ...
  function toStringBench() {
    ...
    if (error) {
      ...
      result += ' x ' + formatNumber(hz.toFixed(hz < 100 ? 2 : 0)) +
          ' ops/sec ' + pm + stats.rme.toFixed(2) + '% (' + size + ' run' +
          (size == 1 ? '' : 's') + ' sampled)';
    }
    return result;
  }
  ...
  function clock() {
    var applet,
        ...
        template = { 'begin': 's$=new n$', 'end': 'r$=(new n$-s$)/1e3',
            'uid': uid },
        timers = [{ 'ns': timer.ns, 'res': max(0.0015, getRes('ms')),
            'unit': 'ms' }];
    ...
    clock = function(clone) {
      ...
      clone.minTime = bench.minTime ||
          (bench.minTime = bench.options.minTime = options.minTime);

      // repair nanosecond timer
      // (some Chrome builds erase the `ns` variable after millions of
      // executions)
      if (applet) {
        try {
          ns.nanoTime();
      ...
      // Create a new compiled test, instead of using the cached
      // `bench.compiled`, to avoid potential engine optimizations enabled over
      // the life of the test.
      var compiled = bench.compiled = createFunction(
        preprocess('t$'),
        interpolate(
          preprocess(deferred
            ? 'var d$=this,#{fnArg}=d$,m$=d$.benchmark._original,f$=m$.fn,' +
              'su$=m$.setup,td$=m$.teardown;' +
              ...
              'd$.teardown=function(){d$.cycles=0;if(typeof td$=="function")' +
            ...
            : 'var r$,s$,m$=this,f$=m$.fn,i$=m$.count,n$=t$.ns;' +
              '#{setup}\n#{begin};' +
              'while(i$--){#{fn}\n}#{end};#{teardown}\nreturn' +
              '{elapsed:r$,uid:"#{uid}"}'
          ),
      ...
      try {
        if (isEmpty) {
          ...
          throw new Error('The test "' + name +
            '" is empty. This may be the result of dead code removal.');
        }
        else if (!deferred) {
          // pretest to determine if compiled code is exits early, usually by a
          // rogue `return` statement, by checking for a return object with the
          // uid
          bench.count = 1;
      ...
      if (decompilable && !compiled && !deferred && !isEmpty) {
        compiled = createFunction(preprocess('t$'), interpolate(
          preprocess(
            ...
            ',n$=t$.ns;#{setup}\n#{begin};m$.f$=f$;while(i$--){m$.f$()}#' +
            '{end};delete m$.f$;#{teardown}\nreturn{elapsed:r$}'
          ),
          source
        ));
      ...
      // assign `compiled` to `clone` before calling in case a deferred
      // benchmark
      ...
      while (count--) {
        if (unit == 'us') {
          ...
          if (ns.stop) {
            ...
            divisor = 1e3;
            measured = Function('n', 'var r,s=n.' + perfName +
              '();while(!(r=n.' + perfName + '()-s)){};return r')(ns);
          } else {
            begin = ns();
  ...
  function compute(bench, options) {
    ...
    function evaluate(event) {
      ...
      if (!done) {
        ...
        // sample standard deviation (estimate of the population standard
        // deviation)
        sd = sqrt(variance);
        // standard error of the mean (a.k.a. the standard deviation of the
  ...
  function cycle(clone, options) {
    ...
    if (clone.running) {
      ...
      if (clone.running) {
        ...
        // calculate how many more iterations it will take to achive the
        // `minTime`
  ...
  function run(options) {
    ...
    if (!event.cancelled) {
      options = {
        'async': ((options = options && options.async) == null
          ? me.async
          : options
        ) && support.timeout
      };
  ...
  // named `options` and Firefox 1 will not assign a value to
  // `Benchmark.options`,
  ...
  extend(Benchmark, {
    ...
    'options': {
      ...
      /**
       * The default number of times to execute a test on a benchmark's first
       * cycle.
       * @memberOf Benchmark.options
       * @type Number
       */
      ...
      /**
       * The time needed to reduce the percent uncertainty of measurement to
       * 1% (secs).
       * @memberOf Benchmark.options
       * @type Number
       */
    ...
    'platform': req('platform') || window.platform || {
      ...
      /**
       * Return platform description when the platform object is coerced to a
       * string.
       * @memberOf Benchmark.platform
       * @type Function
  ...
  extend(Benchmark.prototype, {
    ...
    /**
     * An object of stats including mean, margin or error, and standard
     * deviation.
     * @memberOf Benchmark
     * @type Object
     */
    ...
    /**
     * An object of timing data including cycle, elapsed, period, start,
     * and stop.
     * @memberOf Benchmark
     * @type Object
     */
  ...
  extend(Suite.prototype, {
     ...
     *
     * @memberOf Benchmark.Suite
     * @param {String} [separator=','] A string to separate each element of the
     *        array.
    ...
    /**
     * Retrieves the value of a specified property from all benchmarks in the
     * suite.
     * @memberOf Benchmark.Suite
     * @param {String} property The property to pluck.
     * @returns {Array} A new array of property values.
     ...
     *
     * @memberOf Benchmark.Suite
     * @param {Function} [compareFn=null] A function that defines the sort
     *        order.
