describe('iD.actionCircularize', function () {
    ...
    function isCircular(id, graph) {
        var points = _.map(graph.childNodes(graph.entity(id)), 'loc').map(
                projection),
    ...
    it('limits movement of nodes that are members of other ways', function () {
        ...
        expect(isCircular('-', graph)).to.be.ok;
        expect(iD.geoEuclideanDistance(graph.entity('d').loc, [2, -2]))
            .to.be.lt(0.5);
    });
    ...
    function angle(point1, point2, center) {
        ...
        return 180 / Math.PI * Math.acos(vector1[0] * vector2[0] +
            vector1[1] * vector2[1]);
    }
    ...
    it('creates circle respecting min-angle limit', function() {
        ...
        expect(isCircular('-', graph)).to.be.ok;
        points = _.map(graph.childNodes(graph.entity('-')), 'loc').map(
            projection);
        centroid = d3.polygonCentroid(points);
        ...
        expect(angle(points[points.length - 1], points[0], centroid))
            .to.be.lte(20);
    });
    ...
    it('adds new nodes on shared way wound in opposite direction', function () {
        ...
        expect(_.intersection(graph.entity('-').nodes,
            graph.entity('=').nodes).length).to.eql(3);
        ...
        expect(isCircular('-', graph)).to.be.ok;
        expect(_.intersection(graph.entity('-').nodes,
            graph.entity('=').nodes).length).to.be.gt(3);
        expect(graph.entity('-').isConvex(graph)).to.be.true;
        expect(graph.entity('=').isConvex(graph)).to.be.false;
    });
    ...
    it(
        'circularizes extremely concave ways with a key node on the wrong ' +
        ...
        function () {
            ...
            //    d ------------ e -- g
            //
            //  a-b-c-d-e-a is extremely concave and 'a' is to the left of
            //  centoid..
            var graph = iD.Graph([
                    iD.Node({id: 'a', loc: [ 0,  0]}),
