describe('iD.actionDeleteWay', function() {
    ...
    it('removes a way from parent relations', function() {
        ...
        expect(_.map(graph.entity(relation.id).members, 'id')).
            not.to.contain(way.id);
    });
    ...
    // This was moved to operationDelete.  We should test operations and move
    // this test there.
    // describe('#disabled', function () {
    //     it('returns \'part_of_relation\' for members of route and ' +
    //        'boundary relations',
    //        function () {
    //         var a        = iD.Way({id: 'a'}),
    //             b        = iD.Way({id: 'b'}),
    //             route    = iD.Relation(
    //                {members: [{id: 'a'}], tags: {type: 'route'}}),
    //             boundary = iD.Relation(
    //                {members: [{id: 'b'}], tags: {type: 'boundary'}}),
    //             graph    = iD.Graph([a, b, route, boundary]);
    //         expect(iD.actionDeleteWay('a').disabled(graph))
    //            .to.equal('part_of_relation');
    //         expect(iD.actionDeleteWay('b').disabled(graph))
    //            .to.equal('part_of_relation');
    //     });
    ...
    //        function () {
    //         var way      = iD.Way({id: 'w'}),
    //             relation = iD.Relation({members: [{id: 'w', role: 'outer'}],
    ...
    //         var way      = iD.Way({id: 'w'}),
    //             relation = iD.Relation({members: [{id: 'w', role: 'inner'}],
    //                 tags: {type: 'multipolygon'}}),
    //             graph    = iD.Graph([way, relation]),
    //             action   = iD.actionDeleteWay(way.id);
