describe('iD.actionDisconnect', function () {
    describe('#disabled', function () {
        it('returns \'not_connected\' for a node shared by less than two ways',
            function () {
                ...
                expect(iD.actionDisconnect('a').disabled(graph))
                    .to.equal('not_connected');
            }
        );
        ...
        it(
            'returns falsy for an intersection of two ways with parent way' +
                ' specified',
            function () {
                //    a ---- b ---- c
                //           |
                ...
                expect(iD.actionDisconnect('b', ['|']).disabled(graph))
                    .not.to.be.ok;
            }
        );

        it(
            'returns \'relation\' for a node connecting any two members of' +
                ' the same relation',
            function () {
        ...
        it(
            'returns falsy for a node connecting two members of an unaffected' +
                ' relation',
            function () {
                // a ---- b ---- c
                //        |
                ...
                expect(iD.actionDisconnect('b').limitWays(['|'])
                    .disabled(graph)).not.to.be.ok;
            }
        );
    });
    ...
    it('disconnects a shared non-closing node in an area', function() {
        ...
        var graph = iD.Graph([
            ...
            iD.Node({id: 'd'}),
            iD.Node({id: 'e'}),
            iD.Way({id: 'w', nodes: ['a', 'b', 'c', 'd', 'e', 'b', 'a'],
                tags: {area: 'yes'}})
        ]);

        graph = iD.actionDisconnect('b', '*')(graph);

        expect(graph.entity('w').nodes).to.eql(
            ['a', 'b', 'c', 'd', 'e', '*', 'a']);
    });
    ...
    it('disconnects the closing node of an area without breaking the area',
        function() {
            ...
            var graph = iD.Graph([
                ...
                iD.Node({id: 'd'}),
                iD.Node({id: 'e'}),
                iD.Way({id: 'w1', nodes: ['a', 'b', 'c', 'a'],
                    tags: {area: 'yes'}}),
                iD.Way({id: 'w2', nodes: ['b', 'd', 'e', 'b'],
                    tags: {area: 'yes'}})
            ]);
    ...
    it(
        'disconnects multiple closing nodes of multiple areas without' +
            ' breaking the areas',
        function() {
            // Situtation:
            // a --- b --- d
