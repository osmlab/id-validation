describe('iD.actionJoin', function () {
    describe('#disabled', function () {
        ...
        it(
            'returns falsy for more than two ways when connected, ' +
                'regardless of order',
            function () {
                // a --> b ==> c ~~> d
                var graph = iD.Graph([
                ...
                expect(iD.actionJoin(['-', '=', '~']).disabled(graph))
                    .not.to.be.ok;
                expect(iD.actionJoin(['-', '~', '=']).disabled(graph))
                    .not.to.be.ok;
                expect(iD.actionJoin(['=', '-', '~']).disabled(graph))
        ...
        it('returns \'not_eligible\' for non-line geometries', function () {
            ...
            expect(iD.actionJoin(['a']).disabled(graph))
                .to.equal('not_eligible');
        });

        it(
            'returns \'not_adjacent\' for ways that don\'t share the ' +
                'necessary nodes',
            function () {
                // a -- b -- c
                //      |
                ...
                expect(iD.actionJoin(['-', '=']).disabled(graph))
                    .to.equal('not_adjacent');
            }
        );

        it(
            'returns \'restriction\' in situations where a turn restriction ' +
                ' would be damaged (a)',
            function () {
                // a --> b ==> c
                // from: -
        ...
        it(
            'returns falsy in situations where a turn restriction ' +
                'wouldn\'t be damaged (a)',
            function () {
                // a --> b ==> c
                // |
                // d
        ...
        it(
            'returns \'conflicting_tags\' for two entities that have ' +
                'conflicting tags',
            function () {
                var graph = iD.Graph([
                    iD.Node({id: 'a'}),
    ...
    it('merges relations', function () {
        ...
        expect(graph.entity('r1').members).to.eql(
            [{id: '-', role: 'r1', type: 'way'}]);
        expect(graph.entity('r2').members).to.eql(
            [{id: '-', role: 'r2', type: 'way'}]);
    });
});
