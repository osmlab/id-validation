    ...
    describe('non-destuctive merging', function () {
        describe('tags', function() {
            it('doesn\'t merge tags if conflict (local change, remote change)',
                function () {
                    var localTags = {foo: 'foo_local'},      // changed foo
                        ...
                        remote = base.entity('a').update(
                            {tags: remoteTags, version: '2'}),
                        localGraph = makeGraph([local]),
                        remoteGraph = makeGraph([remote]),
                        action = iD.actionMergeRemoteChanges(
                            'a', localGraph, remoteGraph);
            ...
            it('doesn\'t merge tags if conflict (local add, remote add)',
                function () {
                    // same foo, added bar
                    var localTags = {foo: 'foo', bar: 'bar_local'},
                        remoteTags = {foo: 'foo', bar: 'bar_remote'},
                        local = base.entity('a').update({tags: localTags}),
                        remote = base.entity('a').update(
                            {tags: remoteTags, version: '2'}),
                        localGraph = makeGraph([local]),
        ...
        describe('nodes', function () {
            it('doesn\'t merge nodes if location is different', function () {
                var localTags = {foo: 'foo_local'},   // changed foo
                    ...
                    local = base.entity('a').update(
                        {tags: localTags, loc: localLoc}),
                    remote = base.entity('a').update(
                        {tags: remoteTags, loc: remoteLoc, version: '2'}),
                    localGraph = makeGraph([local]),
                    remoteGraph = makeGraph([remote]),
                    action = iD.actionMergeRemoteChanges('a', localGraph,
                        remoteGraph),
                    result = action(localGraph);
        ...
        describe('ways', function () {
            it('merges ways if nodelist is same', function () {
                var localTags = {foo: 'foo_local', area: 'yes'}, // changed foo
                    // same foo, added bar
                    remoteTags = {foo: 'foo', bar: 'bar_remote', area: 'yes'},
                    mergedTags = {foo: 'foo_local', bar: 'bar_remote',
                        area: 'yes'},
                    local = base.entity('w1').update({tags: localTags}),
                    remote = base.entity('w1').update(
                        {tags: remoteTags, version: '2'}),
                    localGraph = makeGraph([local]),
                    remoteGraph = makeGraph([remote]),
                    action = iD.actionMergeRemoteChanges('w1', localGraph,
                        remoteGraph),
                    result = action(localGraph);
            ...
            it('merges ways if nodelist changed only remotely', function () {
                var localTags = {foo: 'foo_local', area: 'yes'},  // changed foo
                    ...
                    mergedTags = {foo: 'foo_local', bar: 'bar_remote',
                        area: 'yes'},
                    // didn't change nodes
                    localNodes = ['p1', 'p2', 'p3', 'p4', 'p1'],
                    // changed nodes
                    remoteNodes = ['p1', 'r2', 'r3', 'p4', 'p1'],
                    local = base.entity('w1').update(
                        {tags: localTags, nodes: localNodes}),
                    remote = base.entity('w1').update(
                        {tags: remoteTags, nodes: remoteNodes, version: '2'}),
                    localGraph = makeGraph([local]),
            ...
            it('merges ways if nodelist changed only locally', function () {
                var localTags = {foo: 'foo_local', area: 'yes'}, // changed foo
                    ...
                    mergedTags = {foo: 'foo_local', bar: 'bar_remote',
                        area: 'yes'},
                    // changed nodes
                    localNodes = ['p1', 'r2', 'r3', 'p4', 'p1'],
                    // didn't change nodes
                    remoteNodes = ['p1', 'p2', 'p3', 'p4', 'p1'],
                    local = base.entity('w1').update(
                        {tags: localTags, nodes: localNodes}),
                    remote = base.entity('w1').update(
                        {tags: remoteTags, nodes: remoteNodes, version: '2'}),
            ...
            it('merges ways if nodelist changes don\'t overlap', function () {
                    // changed foo
                var localTags   = {foo: 'foo_local', area: 'yes'},
                    // same foo, added bar
                    remoteTags  = {foo: 'foo', bar: 'bar_remote', area: 'yes'},
                    mergedTags  = {foo: 'foo_local', bar: 'bar_remote',
                        area: 'yes'},
                    // changed p2 -> r1, r2
                    localNodes  = ['p1', 'r1', 'r2',  'p3',     'p4',     'p1'],
                    // changed p4 -> r3, r4
                    remoteNodes = ['p1',    'p2',     'p3',  'r3', 'r4',  'p1'],
            ...
            it('merges ways if childNode location is same', function () {
                var localLoc = [12, 12],     // moved node
                    ...
                    remote = base.entity('p1').update(
                        {loc: remoteLoc, version: '2'}),
                    localGraph = makeGraph([local]),
                    remoteGraph = makeGraph([remote]),
                    action = iD.actionMergeRemoteChanges('w1', localGraph,
                        remoteGraph),
        ...
        describe('relations', function () {
            it('doesn\'t merge relations if members have changed', function () {
                    // changed foo
                var localTags   = {foo: 'foo_local', type: 'multipolygon'},
                    // same foo, added bar
                    remoteTags  = {foo: 'foo', bar: 'bar_remote',
                        type: 'multipolygon'},
                    // same members
                    localMembers = [{id: 'w1', role: 'outer'},
                        {id: 'w2', role: 'inner'}],
                    // changed inner to w4
                    remoteMembers = [{id: 'w1', role: 'outer'},
                        {id: 'w4', role: 'inner'}],
                    local = base.entity('r').update(
                        {tags: localTags, members: localMembers}),
                    remote = base.entity('r').update({tags: remoteTags,
                        members: remoteMembers, version: '2'}),
                    localGraph = makeGraph([local]),
                    remoteGraph = makeGraph([remote, s1, s2, s3, s4, w4]),
                    action = iD.actionMergeRemoteChanges('r', localGraph,
                        remoteGraph),
                    result = action(localGraph);
            ...
            it(
                'merges relations if members are same and changed tags ' +
                    ' don\'t conflict',
                function () {
                        // changed foo
                    var localTags   = {foo: 'foo_local', type: 'multipolygon'},
    ...
    describe('destuctive merging', function () {
        describe('nodes', function () {
            it('merges nodes with \'force_local\' option', function () {
                var localTags = {foo: 'foo_local'},       // changed foo
                    ...
                    remoteGraph = makeGraph([remote]),
                    action = iD.actionMergeRemoteChanges('a', localGraph,
                        remoteGraph).withOption('force_local'),
                    result = action(localGraph);
            ...
            it('merges nodes with \'force_remote\' option', function () {
                var localTags = {foo: 'foo_local'},       // changed foo
                    ...
                    local = base.entity('a').update({tags: localTags,
                        loc: localLoc}),
                    remote = base.entity('a').update({tags: remoteTags,
                        loc: remoteLoc, version: '2'}),
                    localGraph = makeGraph([local]),
                    remoteGraph = makeGraph([remote]),
                    action = iD.actionMergeRemoteChanges('a', localGraph,
                        remoteGraph).withOption('force_remote'),
                    result = action(localGraph);
        ...
        describe('ways', function () {
            it('merges ways with \'force_local\' option', function () {
                ...
                var localTags   = {foo: 'foo_local', area: 'yes'},
                    // changed foo
                    remoteTags  = {foo: 'foo_remote', area: 'yes'},
                    // changed p2 -> r1, r2
                    localNodes  = ['p1', 'r1', 'r2', 'p3', 'p4', 'p1'],
                    // changed p2 -> r3, r4
                    remoteNodes = ['p1', 'r3', 'r4', 'p3', 'p4', 'p1'],
            ...
            it('keeps only important childNodes when merging', function () {
                    // changed p2 -> r1, r2
                var localNodes  = ['p1', 'r1', 'r2', 'p3', 'p4', 'p1'],
                    // changed p2 -> r3, r4
                    remoteNodes = ['p1', 'r3', 'r4', 'p3', 'p4', 'p1'],
                    // r1 has interesting tags
                    localr1 = r1.update({tags: {highway: 'traffic_signals'}}),
                    local = base.entity('w1').update({nodes: localNodes}),
                    remote = base.entity('w1').update(
                        {nodes: remoteNodes, version: '2'}),
        ...
        describe('relations', function () {
            it('merges relations with \'force_local\' option', function () {
                ...
                var localTags = {foo: 'foo_local', type: 'multipolygon'},
                    // changed foo
                    remoteTags = {foo: 'foo_remote', type: 'multipolygon'},
                    localMembers = [{id: 'w3', role: 'outer'},
                        {id: 'w2', role: 'inner'}],   // changed outer to w3
                    remoteMembers = [{id: 'w1', role: 'outer'},
                        {id: 'w4', role: 'inner'}],  // changed inner to w4
                    local = base.entity('r').update({tags: localTags,
