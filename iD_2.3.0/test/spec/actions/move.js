describe('iD.actionMove', function() {
    ...
    // This was moved to operationMove.  We should test operations and move this
    // test there.
    ...
    //     it('returns \'incomplete_relation\' for an incomplete relation',
    // function() {
    ...
    it('moves all nodes in a way by the given amount', function() {
        var node1  = iD.Node({loc: [0, 0]}),
            ...
            delta  = [2, 3],
            graph  = iD.actionMove([way.id], delta, projection)(
                iD.Graph([node1, node2, way])),
    ...
    it('moves multiple ways', function() {
        var node   = iD.Node({loc: [0, 0]}),
            ...
            delta  = [2, 3],
            graph  = iD.actionMove([way1.id, way2.id], delta, projection)(
                iD.Graph([node, way1, way2])),
            loc    = graph.entity(node.id).loc;
        expect(loc[0]).to.be.closeTo( 1.440, 0.001);
        expect(loc[1]).to.be.closeTo(-2.159, 0.001);
    });
    ...
    it('moves leaf nodes of a relation', function() {
        var node     = iD.Node({loc: [0, 0]}),
            ...
            delta    = [2, 3],
            graph    = iD.actionMove([relation.id], delta, projection)(
                iD.Graph([node, way, relation])),
            loc      = graph.entity(node.id).loc;
        expect(loc[0]).to.be.closeTo( 1.440, 0.001);
        expect(loc[1]).to.be.closeTo(-2.159, 0.001);
    });
