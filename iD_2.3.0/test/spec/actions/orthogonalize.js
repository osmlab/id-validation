    ...
    it('preserves the shape of skinny quads', function () {
        ...
        for (var i = 0; i < tests.length; i++) {
            var graph = iD.Graph([
                ...
                initialWidth = iD.geoSphericalDistance(
                    graph.entity('a').loc, graph.entity('b').loc),
                finalWidth;

            graph = iD.actionOrthogonalize('-', projection)(graph);

            finalWidth = iD.geoSphericalDistance(
                graph.entity('a').loc, graph.entity('b').loc);
    ...
    describe('transitions', function () {
        ...
        it('orthogonalize at t = 0', function() {
           var graph = iD.Graph([
            ...
            expect(graph.entity('-').nodes).to.eql(
                ['a', 'b', 'c', 'd', 'e', 'f', 'a']);
