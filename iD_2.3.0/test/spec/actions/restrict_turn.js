describe('iD.actionRestrictTurn', function() {
    ...
    it('adds a restriction to an unrestricted turn', function() {
        ...
        expect(r.tags).to.eql(
            {type: 'restriction', restriction: 'no_right_turn'});
        expect(_.pick(r.memberByRole('from'), 'id', 'type'))
            .to.eql({id: '=', type: 'way'});
        expect(_.pick(r.memberByRole('via'), 'id', 'type'))
            .to.eql({id: '*', type: 'node'});
        expect(_.pick(r.memberByRole('to'), 'id', 'type'))
            .to.eql({id: '-', type: 'way'});
    });
    ...
    it('splits the from way when necessary (forward)', function() {
        ...
        expect(r.tags).to.eql(
            {type: 'restriction', restriction: 'no_right_turn'});
        expect(_.pick(r.memberByRole('from'), 'id', 'type')).to.eql(
            {id: '=', type: 'way'});
        expect(_.pick(r.memberByRole('via'), 'id', 'type')).to.eql(
            {id: '*', type: 'node'});
        expect(_.pick(r.memberByRole('to'), 'id', 'type')).to.eql(
            {id: '-', type: 'way'});
    });
    ...
    it('splits the from/to way when necessary (vertex closes from/to)',
        function() {
            //
            //  b -- c
            //  |    |
