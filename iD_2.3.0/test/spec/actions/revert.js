describe('iD.actionRevert', function() {
    ...
    describe('reverting way child nodes', function () {
        it('removes new node, updates parent way nodelist', function() {
            ...
            expect(graph.parentWays(n1), 'n1 has w1 as parent way')
                .to.deep.equal([w1_1]);
            expect(graph.parentWays(n2), 'n2 has w1 as parent way')
                .to.deep.equal([w1_1]);
    ...
    describe('reverting relation members', function () {
        it('removes new node, updates parent relation memberlist', function() {
            ...
            expect(graph.parentRelations(n1), 'n1 has r1 as parent relation')
                .to.deep.equal([r1_1]);
            expect(r1_1.members, 'r1 members updated').to.deep.equal(
                r1.members);
        });
        ...
        it('reverts existing node, preserves parent relation memberlist',
            function() {
                ...
                expect(graph.hasEntity('n1'), 'n1 reverted').to.equal(n1);
                expect(graph.hasEntity('n2'), 'n2 unchanged').to.equal(n2);
                expect(graph.parentRelations(n1),
                    'n1 has r1 as parent relation').to.deep.equal([r1_1]);
                expect(graph.parentRelations(n2),
                    'n2 has r1 as parent relation').to.deep.equal([r1_1]);
                expect(r1_1.members, 'r1 members preserved').to.deep.equal(
                    r1.members);
            }
        );
    });
    ...
    describe('reverting parent ways', function () {
        it('removes new way, preserves new and existing child nodes',
            function() {
                ...
                expect(graph.parentWays(n1), 'n1 has no parent ways')
                    .to.be.empty;
                expect(graph.parentWays(n2), 'n-2 has no parent ways')
                    .to.be.empty;
            }
        );
    ...
    describe('reverting parent relations', function () {
        it('removes new relation, preserves new and existing members',
            function() {
                ...
                expect(graph.hasEntity('n1'), 'n1 unchanged').to.equal(n1);
                expect(graph.hasEntity('n-2'), 'n-2 unchanged').to.equal(n2);
                expect(graph.parentRelations(n1), 'n1 has no parent relations')
                    .to.be.empty;
                expect(graph.parentRelations(n2), 'n-2 has no parent relations')
                    .to.be.empty;
        });
        ...
        it('reverts an updated relation, preserves new and existing members',
            function() {
                ...
                expect(graph.hasEntity('n-2'), 'n-2 unchanged').to.equal(n2);
                expect(graph.parentRelations(n1),
                    'n1 has r1 as parent relation').to.deep.equal([r1]);
                expect(graph.parentRelations(n2), 'n-2 has no parent relations')
                    .to.be.empty;
            }
        );
        ...
        it('restores a deleted relation, preserves new and existing members',
            function() {
                ...
                expect(graph.parentRelations(n1),
                    'n1 has r1 as parent relation').to.deep.equal([r1]);
                expect(graph.parentRelations(n2),
                    'n-2 has no parent relations').to.be.empty;
        });
    });
