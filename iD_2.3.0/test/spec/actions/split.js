describe('iD.actionSplit', function () {
    ...
    describe('#disabled', function () {
        ...
        it(
            'returns falsy for an intersection of two ways with parent ' +
                'way specified',
            function () {
                var graph = iD.Graph([
                        iD.Node({id: 'a'}),
                ...
                expect(iD.actionSplit('*').limitWays(['-']).disabled(graph))
                    .not.to.be.ok;
            }
        );
        ...
        it('returns \'not_eligible\' for the first node of a single way',
            function () {
                ...
                expect(iD.actionSplit('a').disabled(graph)).to.equal(
                    'not_eligible');
            }
        );
        ...
        it('returns \'not_eligible\' for the last node of a single way',
            function () {
                ...
                expect(iD.actionSplit('b').disabled(graph)).to.equal(
                    'not_eligible');
            }
        );

        it(
            'returns \'not_eligible\' for an intersection of two ways ' +
                'with non-parent way specified',
            function () {
                var graph = iD.Graph([
                        iD.Node({id: 'a'}),
                        iD.Node({id: 'b'}),
                ...
                expect(iD.actionSplit('*').limitWays(['-', '='])
                    .disabled(graph)).to.equal('not_eligible');
        });
    });
    ...
    it('splits the specified ways at an intersection', function () {
        ...
        var g3 = iD.actionSplit('*', ['=', '\u00A6']).limitWays(['-', '|'])(
            graph);
    ...
    it('splits a closed way at the given point and its antipode', function () {
        ...
        //
        var graph = iD.Graph([
                iD.Node({id: 'a', loc: [0,1]}),
                iD.Node({id: 'b', loc: [1,1]}),
                iD.Node({id: 'c', loc: [1,0]}),
    ...
    it('adds the new way to parent relations (no connections)', function () {
        ...
        var graph = iD.Graph([
                ...
                iD.Way({id: '-', nodes: ['a', 'b', 'c']}),
                iD.Relation({id: 'r',
                    members: [{id: '-', type: 'way', role: 'forward'}]})
            ]);
    ...
    it('adds the new way to parent relations (forward order)', function () {
        ...
        var graph = iD.Graph([
                ...
                iD.Way({id: '~', nodes: ['c', 'd']}),
                iD.Relation({id: 'r',
                    members: [{id: '-', type: 'way'}, {id: '~', type: 'way'}]})
            ]);
    ...
    it('converts simple multipolygon to a proper multipolygon', function () {
        var graph = iD.Graph([
                ...
                iD.Relation({id: 'r', members: [{id: '-', type: 'way',
                    role: 'outer'}], tags: {type: 'multipolygon'}})
            ]);

        graph = iD.actionSplit('b', ['='])(graph);

        expect(graph.entity('-').tags).to.eql({});
        expect(graph.entity('r').tags).to.eql({type: 'multipolygon',
            natural: 'water'});
        expect(_.map(graph.entity('r').members, 'id')).to.eql(['-', '=']);
    });
