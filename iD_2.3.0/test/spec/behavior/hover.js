describe('iD.behaviorHover', function() {
    ...
    describe('#off', function () {
        ...
        it('removes the .hover-disabled class from the surface element',
            function () {
    ...
    describe('mouseover', function () {
        it(
            'adds the .hover class to all elements to which the same datum ' +
                'is bound',
            function () {
                var a = iD.Node({id: 'a'}),
                    b = iD.Node({id: 'b'});
                ...
                expect(container.selectAll('.a.hover').nodes())
                    .to.have.length(2);
                expect(container.selectAll('.b.hover').nodes())
                    .to.have.length(0);
        });
    ...
    describe('alt keydown', function () {
        it('replaces the .hover class with .hover-suppressed', function () {
            ...
            expect(container.selectAll('.hover').nodes()).to.have.length(0);
            expect(container.selectAll('.hover-suppressed').nodes())
                .to.have.length(1);
