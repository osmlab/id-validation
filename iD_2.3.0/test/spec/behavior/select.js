describe('iD.behaviorSelect', function() {
    ...
    specify('shift-click on unselected entity adds it to the selection',
        function() {
            context.enter(iD.modeSelect(context, [a.id]));
    ...
    specify('shift-click on last selected entity clears the selection',
        function() {
            context.enter(iD.modeSelect(context, [a.id]));
            var el = context.surface().selectAll('.' + a.id).node();
            happen.mousedown(el, { shiftKey: true });
    ...
    specify('shift-click on empty space leaves the selection unchanged',
        function() {
            context.enter(iD.modeSelect(context, [a.id]));
            var el = context.surface().node();
            happen.mousedown(el, { shiftKey: true });
