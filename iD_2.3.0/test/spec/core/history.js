describe('iD.History', function () {
    ...
    describe('#toJSON', function() {
        ...
        it('generates v3 JSON', function() {
            ...
            // modification
            history.perform(iD.actionChangeTags('n2', {k: 'v'}));
            history.perform(iD.actionDeleteNode('n3')); // deletion
            var json = JSON.parse(history.toJSON());
            expect(json.version).to.eql(3);
            expect( _.isEqual(json.entities,
                [node_1, node2.update({tags: {k: 'v'}})]) ).to.be.ok;
    ...
    describe('#fromJSON', function() {
            ...
            var json = {
                'stack': [
                    ...
                    {'entities': {'n-1': {'loc': [1, 2], 'id': 'n-1'}},
                        'imageryUsed': ['Bing'], 'annotation': 'Added a point.'
                    }
                ],
        ...
        it('restores from v1 JSON (modification)', function() {
            var json = {
                'stack': [
                    ...
                    {'entities': {'n-1': {'loc': [2, 3], 'id': 'n-1', 'v': 1}},
                        'imageryUsed': ['Bing'], 'annotation': 'Moved a point.'
                    }
                ],
                'nextIDs': {'node': -2, 'way': -1, 'relation': -1},
                'index': 2
            ...
            history.fromJSON(JSON.stringify(json));
            expect(history.graph().entity('n-1')).to.eql(iD.Node({id: 'n-1',
                loc: [2, 3], v: 1}));
        ...
        it('restores from v1 JSON (deletion)', function() {
            var json = {
                'stack': [
                    {'entities': {}},
                    {'entities': {'n1': 'undefined'}, 'imageryUsed': ['Bing'],
        ...
        it('restores from v2 JSON (creation)', function() {
            var json = {
                ...
                'stack': [
                    {},
                    {'modified': ['n-1v0'], 'imageryUsed': ['Bing'],
                        'annotation': 'Added a point.'
                    }
                ],
                'nextIDs': {'node': -2, 'way': -1, 'relation': -1},
        ...
        it('restores from v2 JSON (modification)', function() {
            var json = {
                ...
                'stack': [
                    {},
                    {'modified': ['n1v1'], 'imageryUsed': ['Bing'],
            ...
            history.fromJSON(JSON.stringify(json));
            // Shouldn't be necessary; flaw in v2 format (see #2135)
            history.merge([iD.Node({id: 'n1'})]);
            expect(history.graph().entity('n1')).to.eql(
                iD.Node({id: 'n1', loc: [2, 3], v: 1}));
            expect(history.undoAnnotation()).to.eql('Moved a point.');
            expect(history.imageryUsed()).to.eql(['Bing']);
            expect(iD.Entity.id.next).to.eql({node: -2, way: -1, relation: -1});
