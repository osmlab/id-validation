describe('iD.Tree', function() {
    describe('#rebase', function() {
        it('adds entities to the tree', function() {
            ...
            expect(tree.intersects(iD.geoExtent([0, 0], [2, 2]), graph))
                .to.eql([node]);
        });
        ...
        it('does not insert if entity has a modified version', function() {
            ...
            expect(tree.intersects(iD.geoExtent([9, 9], [11, 11]), g))
                .to.eql([node_]);
            ...
            expect(tree.intersects(iD.geoExtent([0, 0], [11, 11]), g))
                .to.eql([node_]);
        });
    ...
    describe('#intersects', function() {
        ...
        it(
            'includes intersecting relations after incomplete members are ' +
                'loaded',
            function() {
                var graph = iD.Graph(),
                    tree = iD.Tree(graph),
        ...
        // This happens when local storage includes a changed way but not its
        // nodes.
        ...
        it(
            'adjusts parent relations of parent ways when a member node is ' +
                'moved',
            function() {
                var graph = iD.Graph(),
                    tree = iD.Tree(graph),
                    ...
                    relation = iD.Relation(
                        {members: [{type: 'multipolygon', id: 'w'}]}),
                    extent = iD.geoExtent([0, 0], [2, 2]);
        ...
        it(
            'don\'t include parent way multiple times when multiple ' +
                'child nodes are moved',
            function() {
                ...
                graph = graph.replace(n1.move([1.1,1.1])).replace(
                    n2.move([2.1,2.1]));
                expect(
