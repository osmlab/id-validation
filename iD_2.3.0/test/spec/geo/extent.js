describe('iD.geoExtent', function () {
    describe('constructor', function () {
        it('defaults to infinitely empty extent', function () {
            expect(iD.geoExtent().equals(
                [[Infinity, Infinity], [-Infinity, -Infinity]])).to.be.ok;
        });
    ...
    describe('#rectangle', function () {
        it('returns the extent as a rectangle', function () {
            expect(iD.geoExtent([0, 0], [5, 10]).rectangle())
                .to.eql([0, 0, 5, 10]);
        });
    });
    ...
    describe('#padByMeters', function () {
        it('does not change centerpoint of an extent', function () {
           ...
           expect(iD.geoExtent(min, max).padByMeters(100).center())
              .to.eql([2.5, 5]);
        });
    ...
    describe('#extend', function () {
        ...
        it('returns the minimal extent containing self and the given point',
            function () {
                expect(iD.geoExtent().extend([0, 0]).equals([[0, 0], [0, 0]]))
                    .to.be.ok;
                expect(iD.geoExtent([0, 0], [0, 0]).extend([5, 10])
                    .equals([[0, 0], [5, 10]])).to.be.ok;
            }
        );

        it('returns the minimal extent containing self and the given extent',
            function () {
                expect(iD.geoExtent().extend([[0, 0], [5, 10]])
                    .equals([[0, 0], [5, 10]])).to.be.ok;
                expect(iD.geoExtent([0, 0], [0, 0]).extend([[4, -1], [5, 10]])
                    .equals([[0, -1], [5, 10]])).to.be.ok;
            }
        );
    });
    ...
    describe('#_extend', function () {
        it(
            'extends self to the minimal extent containing self and the ' +
                'given extent',
            function () {
                var e = iD.geoExtent();
                e._extend([[0, 0], [5, 10]]);
    ...
    describe('#contains', function () {
        ...
        it('returns true for an extent contained by self',
            function () {
                expect(iD.geoExtent([0, 0], [5, 5]).contains([[1, 1], [2, 2]]))
                    .to.be.true;
                expect(iD.geoExtent([1, 1], [2, 2]).contains([[0, 0], [5, 5]]))
                    .to.be.false;
            }
        );
        ...
        it('returns false for an extent partially contained by self',
            function () {
                expect(iD.geoExtent([0, 0], [5, 5]).contains([[1, 1], [6, 6]]))
                    .to.be.false;
                expect(iD.geoExtent([1, 1], [6, 6]).contains([[0, 0], [5, 5]]))
                    .to.be.false;
            }
        );

        it('returns false for an extent not intersected by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).contains([[6, 6], [7, 7]]))
                .to.be.false;
            expect(iD.geoExtent([[6, 6], [7, 7]]).contains([[0, 0], [5, 5]]))
                .to.be.false;
        });
    });
    ...
    describe('#intersects', function () {
        ...
        it('returns true for an extent contained by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).intersects([[1, 1], [2, 2]]))
                .to.be.true;
            expect(iD.geoExtent([1, 1], [2, 2]).intersects([[0, 0], [5, 5]]))
                .to.be.true;
        });

        it('returns true for an extent partially contained by self',
            function () {
                expect(iD.geoExtent([0, 0], [5, 5])
                    .intersects([[1, 1], [6, 6]])).to.be.true;
                expect(iD.geoExtent([1, 1], [6, 6])
                    .intersects([[0, 0], [5, 5]])).to.be.true;
            }
        );

        it('returns false for an extent not intersected by self', function () {
            expect(iD.geoExtent([0, 0], [5, 5]).intersects([[6, 6], [7, 7]]))
                .to.be.false;
            expect(iD.geoExtent([[6, 6], [7, 7]]).intersects([[0, 0], [5, 5]]))
                .to.be.false;
        });
    });
