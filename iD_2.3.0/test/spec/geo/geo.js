describe('iD.geo', function() {
    ...
    describe('geoOffsetToMeters', function() {
        ...
        it(
            '[0.00064, -0.00064] pixel offset is roughly [100, 100] meter ' +
                'offset',
            function() {
    ...
    describe('geoMetersToOffset', function() {
        ...
        it(
            '[100, 100] meter offset is roughly [0.00064, -0.00064] pixel ' +
                'offset',
            function() {
    ...
    describe('geoSphericalDistance', function() {
        ...
        it('a straight 1 degree line at the equator is aproximately 111 km',
            function() {
                var a = [0, 0],
                    b = [1, 0];
                expect(iD.geoSphericalDistance(a, b)).to.be.within(
                    110E3, 112E3);
            }
        );
    ...
    describe('geoAngle', function() {
        it('returns angle between a and b', function() {
            ...
            expect(iD.geoAngle({loc:[0, 0]}, {loc:[1, 0]}, projection))
                .to.be.closeTo(0, 1e-6);
            expect(iD.geoAngle({loc:[0, 0]}, {loc:[0, 1]}, projection))
                .to.be.closeTo(Math.PI / 2, 1e-6);
            expect(iD.geoAngle({loc:[0, 0]}, {loc:[-1, 0]}, projection))
                .to.be.closeTo(Math.PI, 1e-6);
            expect(iD.geoAngle({loc:[0, 0]}, {loc:[0, -1]}, projection))
                .to.be.closeTo(-Math.PI / 2, 1e-6);
        });
    });
    ...
    describe('geoChooseEdge', function() {
        ...
        it('returns undefined properties for a degenerate way (no nodes)',
            function() {
        ...
        it('returns undefined properties for a degenerate way (single node)',
            function() {
                expect(iD.geoChooseEdge([iD.Node({loc: [0, 0]})], [0, 0],
                    projection)).to.eql({
                        index: undefined,
                        distance: Infinity,
                        loc: undefined
    ...
    describe('geoPolygonIntersectsPolygon', function() {
        ...
        it(
            'returns true when outer polygon partially contains inner '+
                '(some vertices contained)',
            function() {
                var outer = [[0, 0], [0, 3], [3, 3], [3, 0], [0, 0]];
                var inner = [[-1, -1], [1, 2], [2, 2], [2, 1], [1, 1]];
                expect(iD.geoPolygonIntersectsPolygon(outer, inner)).to.be.true;
        ...
        it(
            'returns false when outer polygon partially contains inner ' +
                '(no vertices contained - lax test)',
            function() {
                var outer = [[0, 0], [0, 3], [3, 3], [3, 0], [0, 0]];
                var inner = [[1, -1], [1, 4], [2, 4], [2, -1], [1, -1]];
                expect(iD.geoPolygonIntersectsPolygon(outer, inner))
                    .to.be.false;
            }
        );
        ...
        it(
            'returns true when outer polygon partially contains inner ' +
                '(no vertices contained - strict test)',
            function() {
                var outer = [[0, 0], [0, 3], [3, 3], [3, 0], [0, 0]];
                var inner = [[1, -1], [1, 4], [2, 4], [2, -1], [1, -1]];
                expect(iD.geoPolygonIntersectsPolygon(outer, inner, true))
                    .to.be.true;
            }
        );
