describe('d3.keybinding', function() {
    ...
    describe('#on', function () {
        ...
        it('ignores alt and control if both are present (e.g. as AltGr) #4096',
            function () {
                d3.select(document).call(keybinding.on('A', spy));

                happen.keydown(document,
                    {keyCode: 65, altKey: true, ctrlKey: true});
                expect(spy).to.have.been.calledOnce;
            }
        );
        ...
        it('does not dispatch when focus is in input elements by default',
            function () {
                d3.select(document).call(keybinding.on('A', spy));
        ...
        it(
            'dispatches when focus is in input elements when the capture ' +
                'flag was passed',
            function () {
