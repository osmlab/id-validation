describe('iD.osmEntity', function () {
    ...
    describe('.id', function () {
        ...
        describe('.toOSM', function () {
            it('reverses fromOSM', function () {
                expect(iD.Entity.id.toOSM(iD.Entity.id.fromOSM('node', '1')))
                    .to.equal('1');
            });
        });
    });
    ...
    describe('#intersects', function () {
        it('returns true for a way with a node within the given extent',
            function () {
                ...
                expect(way.intersects([[-5, -5], [5, 5]], graph))
                    .to.equal(true);
            }
        );
    ...
    describe('#hasDeprecatedTags', function () {
        ...
        it('returns true if entity has deprecated tags', function () {
            expect(iD.Entity({ tags: { barrier: 'wire_fence' } })
                .deprecatedTags()).to.eql({ barrier: 'wire_fence' });
        });
    });
    ...
    describe('#hasInterestingTags', function () {
        ...
        it(
            'returns true if the entity has tags other than \'attribution\',
                \'created_by\', \'source\', \'odbl\' and tiger tags',
            function () {
                expect(iD.Entity({tags: {foo: 'bar'}}).hasInterestingTags())
                    .to.equal(true);
            }
        );
        ...
        it('return false if the entity has only uninteresting tags',
            function () {
                expect(iD.Entity({tags: {source: 'Bing'}}).hasInterestingTags())
                    .to.equal(false);
            }
        );

        it('return false if the entity has only tiger tags', function () {
            expect(iD.Entity({tags: {'tiger:source': 'blah',
                'tiger:foo': 'bar'}}).hasInterestingTags()).to.equal(false);
        });
    });
