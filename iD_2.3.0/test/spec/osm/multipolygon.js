describe('iD.osmIsSimpleMultipolygonOuterMember', function() {
    it('returns the parent relation of a simple multipolygon outer', function(){
        ...
        expect(iD.osmIsSimpleMultipolygonOuterMember(outer, graph))
            .to.equal(relation);
    });

    it(
        'returns the parent relation of a simple multipolygon outer, assuming' +
            ' role outer if unspecified',
        function() {
            var outer = iD.Way({tags: {'natural':'wood'}}),
                relation = iD.Relation({tags: {type: 'multipolygon'},
                    members: [{id: outer.id}]}),
    ...
    it('returns false if the parent has interesting tags', function() {
        var outer = iD.Way({tags: {'natural':'wood'}}),
            relation = iD.Relation({
                tags: {natural: 'wood', type: 'multipolygon'},
                members: [{id: outer.id, role: 'outer'}]}
            ),
            graph = iD.Graph([outer, relation]);
        expect(iD.osmIsSimpleMultipolygonOuterMember(outer, graph)).to.be.false;
    ...
    it(
        'returns the parent relation of a simple multipolygon outer, ' +
            'ignoring uninteresting parent tags',
        function() {
            var outer = iD.Way({tags: {'natural':'wood'}}),
                relation = iD.Relation({
                    tags: {'tiger:reviewed':'no', type: 'multipolygon'},
                    members: [{id: outer.id, role: 'outer'}]}
                ),
                graph = iD.Graph([outer, relation]);
            expect(iD.osmIsSimpleMultipolygonOuterMember(outer, graph))
    ...
    it('returns false if the parent has multiple outer ways', function() {
        ...
        expect(iD.osmIsSimpleMultipolygonOuterMember(outer1, graph))
            .to.be.false;
        expect(iD.osmIsSimpleMultipolygonOuterMember(outer2, graph))
            .to.be.false;
    });

    it(
        'returns false if the parent has multiple outer ways, assuming role '+
            'outer if unspecified',
        function() {
            var outer1 = iD.Way({tags: {'natural':'wood'}}),
                outer2 = iD.Way({tags: {'natural':'wood'}}),
                relation = iD.Relation({tags: {type: 'multipolygon'},
...
describe('iD.osmSimpleMultipolygonOuterMember', function() {
    it('returns the outer member of a simple multipolygon', function() {
        ...
        expect(iD.osmSimpleMultipolygonOuterMember(inner, graph))
            .to.equal(outer);
        expect(iD.osmSimpleMultipolygonOuterMember(outer, graph))
            .to.equal(outer);
    });
...
describe('iD.osmJoinWays', function() {
    ...
    it('reverses member tags of reversed segements', function() {
        ...
        var graph = iD.Graph([
                ...
                iD.Way({id: '-', nodes: ['a', 'b']}),
                iD.Way({id: '=', nodes: ['c', 'b'],
                    tags: {'oneway': 'yes', 'lanes:forward': 2}})
            ]);

        var result = iD.osmJoinWays(
