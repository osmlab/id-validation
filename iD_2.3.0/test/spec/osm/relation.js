describe('iD.osmRelation', function () {
    ...
    describe('#geometry', function () {
        it('returns \'area\' for multipolygons', function () {
            expect(iD.Relation({tags: {type: 'multipolygon'}}).geometry(
                iD.Graph())).to.equal('area');
        });
    ...
    describe('#isDegenerate', function () {
        ...
        it('returns false for a relation with members', function () {
            expect(iD.Relation({members: [{id: 'a', role: 'inner'}]})
                .isDegenerate()).to.equal(false);
        });
    });
    ...
    describe('#memberByRole', function () {
        it('returns the first member with the given role', function () {
            var r = iD.Relation({members: [
                ...
                {id: 'b', role: 'outer'},
                {id: 'c', role: 'outer'}]});
            expect(r.memberByRole('outer')).to.eql(
                {id: 'b', role: 'outer', index: 1});
        });
    ...
    describe('#isRestriction', function () {
        it('returns true for \'restriction\' type', function () {
            expect(iD.Relation({tags: {type: 'restriction'}}).isRestriction())
                .to.be.true;
        });

        it('returns true for \'restriction:type\' types', function () {
            expect(iD.Relation({tags: {type: 'restriction:bus'}})
                .isRestriction()).to.be.true;
        });

        it('returns false otherwise', function () {
            expect(iD.Relation().isRestriction()).to.be.false;
            expect(iD.Relation({tags: {type: 'multipolygon'}}).isRestriction())
                .to.be.false;
        });
    });
    ...
    describe('#indexedMembers', function () {
        it('returns an array of members extended with indexes', function () {
            ...
            expect(r.indexedMembers()).to.eql(
                [{id: '1', index: 0}, {id: '3', index: 1}]);
        });
    });
    ...
    describe('#addMember', function () {
        ...
        it('adds a member at index 0', function () {
            ...
            expect(r.addMember({id: '2'}, 0).members).to.eql(
                [{id: '2'}, {id: '1'}]);
        });

        it('adds a member at a positive index', function () {
            var r = iD.Relation({members: [{id: '1'}, {id: '3'}]});
            expect(r.addMember({id: '2'}, 1).members).to.eql(
                [{id: '1'}, {id: '2'}, {id: '3'}]);
        });
    ...
    describe('#updateMember', function () {
        it(
            'updates the properties of the relation member at the ' +
                'specified index',
            function () {
                var r = iD.Relation({members: [{role: 'forward'}]});
                expect(r.updateMember({role: 'backward'}, 0).members)
                    .to.eql([{role: 'backward'}]);
            }
        );
    });
    ...
    describe('#replaceMember', function () {
        ...
        it('removes members if replacing them would produce duplicates',
            function () {
                ...
                expect(r.replaceMember({id: 'a'}, {id: 'b', type: 'node'})
                    .members).to.eql([{id: 'b', role: 'b', type: 'node'}]);
            }
        );
    });
    ...
    describe('#asJXON', function () {
        it('converts a relation to jxon', function() {
            var relation = iD.Relation({id: 'r-1', members: [{id: 'w1',
                    role: 'forward', type: 'way'}], tags: {type: 'route'}});
        ...
        it('includes changeset if provided', function() {
            expect(iD.Relation().asJXON('1234').relation['@changeset'])
                .to.equal('1234');
        });
    });

    describe('#asGeoJSON', function (){
        it('converts a multipolygon to a GeoJSON MultiPolygon geometry',
            function() {
                var a = iD.Node({loc: [1, 1]}),
                    b = iD.Node({loc: [3, 3]}),
                    c = iD.Node({loc: [2, 2]}),
                    w = iD.Way({nodes: [a.id, b.id, c.id, a.id]}),
                    r = iD.Relation({tags: {type: 'multipolygon'},
        ...
        it('forces clockwise winding order for outer multipolygon ways',
            function() {
                var a = iD.Node({loc: [0, 0]}),
                    b = iD.Node({loc: [0, 1]}),
                    c = iD.Node({loc: [1, 0]}),
                ...
                expect(json.coordinates[0][0]).to.eql(
                    [a.loc, b.loc, c.loc, a.loc]);
        });

        it('forces counterclockwise winding order for inner multipolygon ways',
            function() {
                var a = iD.Node({loc: [0, 0]}),
                    b = iD.Node({loc: [0, 1]}),
                    c = iD.Node({loc: [1, 0]}),
        ...
        it('converts a relation to a GeoJSON FeatureCollection', function() {
            ...
            expect(json.features).to.eql([_.extend({role: 'role'},
                a.asGeoJSON(g))]);
        });
    });
    ...
    describe('#multipolygon', function () {
        ...
        specify('single polygon consisting of multiple ways', function () {
            var a = iD.Node({loc: [1, 1]}),
                ...
                r  = iD.Relation({members: [{id: w1.id, type: 'way'},
                    {id: w2.id, type: 'way'}]}),
        ...
        specify(
            'single polygon consisting of multiple ways, one needing ' +
                'reversal',
            function () {
                var a  = iD.Node({loc: [1, 1]}),
                    b  = iD.Node({loc: [3, 3]}),
                ...
                expect(r.multipolygon(g)).to.eql(
                    [[[a.loc, b.loc, c.loc, a.loc]]]);
        });
        ...
        specify('invalid geometry: unclosed ring consisting of a single way',
            function () {
                var a = iD.Node({loc: [1, 1]}),
                    b = iD.Node({loc: [3, 3]}),
                    c = iD.Node({loc: [2, 2]}),
        ...
        specify('invalid geometry: unclosed ring consisting of multiple ways',
            function () {
                var a  = iD.Node({loc: [1, 1]}),
                    b  = iD.Node({loc: [3, 3]}),
                    c  = iD.Node({loc: [2, 2]}),
        ...
        specify(
            'invalid geometry: unclosed ring consisting of multiple ' +
                'ways, alternate order',
            function () {
                var a  = iD.Node({loc: [1, 1]}),
                    b  = iD.Node({loc: [2, 2]}),
                    c  = iD.Node({loc: [3, 3]}),
                ...
                expect(r.multipolygon(g)).to.eql(
                    [[[d.loc, c.loc, b.loc, a.loc]]]);
            }
        );

        specify(
            'invalid geometry: unclosed ring consisting of multiple ' +
                'ways, one needing reversal',
            function () {
                var a  = iD.Node({loc: [1, 1]}),
                    b  = iD.Node({loc: [2, 2]}),
                    c  = iD.Node({loc: [3, 3]}),
        ...
        specify(
            'invalid geometry: unclosed ring consisting of multiple ' +
                'ways, one needing reversal, alternate order',
            function () {
                var a  = iD.Node({loc: [1, 1]}),
                    b  = iD.Node({loc: [2, 2]}),
                    c  = iD.Node({loc: [3, 3]}),
        ...
        specify('single polygon with single multi-way inner', function () {
            ...
            expect(r.multipolygon(graph)).to.eql([[[a.loc, b.loc, c.loc, a.loc],
                [d.loc, e.loc, f.loc, d.loc]]]);
        });
        ...
        specify('single polygon with multiple single-way inners', function () {
            var a = iD.Node({loc: [0, 0]}),
                ...
                r = iD.Relation({members: [
                    ...
                    {id: inner1.id, role: 'inner', type: 'way'},
                    {id: inner2.id, role: 'inner', type: 'way'}]}),
                graph = iD.Graph([a, b, c, d, e, f, g, h, i, outer, inner1,
                    inner2, r]);

            expect(r.multipolygon(graph)).to.eql([[[a.loc, b.loc, c.loc, a.loc],
                [d.loc, e.loc, f.loc, d.loc], [g.loc, h.loc, i.loc, g.loc]]]);
        });
        ...
        specify('multiple polygons with single single-way inner', function () {
            var a = iD.Node({loc: [0, 0]}),
                ...
                r = iD.Relation({members: [
                    ...
                    {id: inner.id, role: 'inner', type: 'way'}]}),
                graph = iD.Graph([a, b, c, d, e, f, g, h, i, outer1, outer2,
                    inner, r]);
        ...
        specify('invalid geometry: unmatched inner', function () {
            var a = iD.Node({loc: [1, 1]}),
                ...
                c = iD.Node({loc: [3, 3]}),
                w = iD.Way({nodes: [a.id, b.id, c.id, a.id]}),
                r = iD.Relation({members: [{id: w.id, role: 'inner',
                    type: 'way'}]}),
                g = iD.Graph([a, b, c, w, r]);
        ...
        specify('incomplete relation', function () {
            var a = iD.Node({loc: [1, 1]}),
                ...
                w1 = iD.Way({nodes: [a.id, b.id, c.id]}),
                w2 = iD.Way(),
                r  = iD.Relation({members: [{id: w2.id, type: 'way'},
                    {id: w1.id, type: 'way'}]}),
