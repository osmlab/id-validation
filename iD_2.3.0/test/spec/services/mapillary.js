describe('iD.serviceMapillary', function() {
       ...
       it('loads sign_defs', function() {
            ...
            server.respondWith('GET', match, function (xhr) {
                xhr.respond(200, { 'Content-Type': 'application/json' },
                    '{ "' + sign + '": { "height": 24, "pixelRatio": 1, ' +
                    '"width": 24, "x": 576, "y": 528} }');
            });
            server.respond();

            var sign_defs = mapillary.signDefs();

            expect(sign_defs).to.have.property(
        ...
        it('fires loadedSigns when signs are loaded', function() {
            ...
            var match = /objects/,
                ...
                features = [{
                    type: 'Feature',
                    geometry: { type: 'Point', coordinates: [10,0] },
                    properties: { detections: detections, key: '0',
                        value: 'not-in-set' }
                }],
                response = { type: 'FeatureCollection', features: features };
    ...
    describe('#images', function() {
        it('returns images in the visible map area', function() {
            var features = [
                { minX: 10, minY: 0, maxX: 10, maxY: 0,
                    data: { key: '0', loc: [10,0], ca: 90 } },
                { minX: 10, minY: 0, maxX: 10, maxY: 0,
                    data: { key: '1', loc: [10,0], ca: 90 } },
                { minX: 10, minY: 1, maxX: 10, maxY: 1,
                    data: { key: '2', loc: [10,1], ca: 90 } }
            ];
        ...
        it('limits results no more than 3 stacked images in one spot',
            function() {
                var features = [
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '0', loc: [10,0], ca: 90 } },
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '1', loc: [10,0], ca: 90 } },
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '2', loc: [10,0], ca: 90 } },
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '3', loc: [10,0], ca: 90 } },
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '4', loc: [10,0], ca: 90 } }
                ];
    ...
    describe('#signs', function() {
        it('returns signs in the visible map area', function() {
            var signs = [{
                ...
                features = [
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '0', loc: [10,0], signs: signs } },
                    { minX: 10, minY: 0, maxX: 10, maxY: 0,
                        data: { key: '1', loc: [10,0], signs: signs } },
                    { minX: 10, minY: 1, maxX: 10, maxY: 1,
                        data: { key: '2', loc: [10,1], signs: signs } }
                ];
        ...
        it('limits results no more than 3 stacked signs in one spot',
            function() {
                var signs = [{
                        'package': 'trafficsign_us_3.0',
                        rect: [ 0.805, 0.463, 0.833, 0.502 ],
                    ...
                    features = [
                        ...
                        { minX: 10, minY: 0, maxX: 10, maxY: 0,
                            data: { key: '1', loc: [10,0], signs: signs } },
                        { minX: 10, minY: 0, maxX: 10, maxY: 0,
                            data: { key: '2', loc: [10,0], signs: signs } },
                        { minX: 10, minY: 0, maxX: 10, maxY: 0,
                            data: { key: '3', loc: [10,0], signs: signs } },
                        { minX: 10, minY: 0, maxX: 10, maxY: 0,
                            data: { key: '4', loc: [10,0], signs: signs } }
                    ];
    ...
    describe('#signHTML', function() {
        it('returns sign HTML', function() {
            ...
            expect(mapillary.signHTML(signdata)).to.eql(
                '<div style="background-image:url(' + sprite +
                ');background-repeat:no-repeat;height:24px;width:24px;' +
                'background-position-x:-576px;background-position-y:-528px">' +
                '</div>'
            );
        });
    });
