describe('iD.serviceNominatim', function() {
    ...
    describe('#countryCode', function() {
        it(
            'calls the given callback with the results of the country ' +
                'code query',
            function() {
                ...
                expect(query(server.requests[0].url)).to.eql(
                    {zoom: '13', format: 'json',
                        addressdetails: '1', lat: '48', lon: '16'});
    ...
    describe('#reverse', function() {
        it('should not cache distant result', function() {
            ...
            expect(query(server.requests[0].url)).to.eql(
                {zoom: '13', format: 'json', addressdetails: '1',
                    lat: '49', lon: '17'});
            expect(callback).to.have.been.calledWithExactly(null,
                {address: {country_code:'cz'}});
        });
            ...
            function() {
                ...
                server.respondWith('GET', new RegExp('https://nominatim.openstreetmap.org/search'),
                    [
                        ...
                        '[{"place_id":"158484588","osm_type":"relation",' +
                            '"osm_id":"188022","boundingbox":["39.867005",' +
                            '"40.1379593","-75.2802976","-74.9558313"],"lat"' +
                            ':"39.9523993","lon":"-75.1635898","display_name"' +
                            ':"Philadelphia, Philadelphia County, ' +
                            'Pennsylvania, United States of America","class"' +
                            ':"place","type":"city","importance":'+
                            '0.83238050437778}]'
                    ]);
                server.respond();

                expect(query(server.requests[0].url))
                    .to.eql({format: 'json', limit: '10'});
