    ...
    describe('#loadFromAPI', function () {
        var server,
            ...
            response = '<?xml version="1.0" encoding="UTF-8"?>' +
                '<osm version="0.6">' +
                '  <bounds minlat="40.655" minlon="-74.542" maxlat="40.656" ' +
                'maxlon="-74.541' +
                '  <node id="105340439" visible="true" version="2" ' +
                'changeset="2880013" timestamp="2009-10-18T07:47:39Z" ' +
                'user="woodpeck_fixbot" uid="147510" lat="40.6555" ' +
                'lon="-74.5415"/>' +
                '  <node id="105340442" visible="true" version="2" ' +
                'changeset="2880013" timestamp="2009-10-18T07:47:39Z" ' +
                'user="woodpeck_fixbot" uid="147510" lat="40.6556" ' +
                'lon="-74.5416"/>' +
                '  <way id="40376199" visible="true" version="1" ' +
                'changeset="2403012" timestamp="2009-09-07T16:01:13Z" ' +
                'user="NJDataUploads" uid="148169">' +
                '    <nd ref="105340439"/>' +
                '    <nd ref="105340442"/>' +
                    ...
                    function(request) {
                        if (connection.authenticated()) {
                            ...
                            return request.respond(200,
                                { 'Content-Type': 'text/xml' }, response);
                        }
                    }
                );
                server.respond();
        ...
        it('retries an authenticated call unauthenticated if 403 Forbidden',
            function (done) {
                login();
                connection.loadFromAPI(path, function (err, xml) {
                    expect(err).to.be.not.ok;
        ...
        it('dispatches change event if 509 Bandwidth Limit Exceeded',
            function (done) {
                logout();
                connection.on('change', spy);
                ...
                server.respondWith('GET', 'http://www.openstreetmap.org' + path,
                    function(request) {
                        if (!connection.authenticated()) {
                            // workaround: sinon.js seems to call error handler
                            // with a sinon.Event instead of the target
                            // XMLHttpRequest object..
                            var orig = request.onreadystatechange;
                            request.onreadystatechange =
                                function(o) { orig((o && o.target) || o); };
    ...
    describe('#loadEntity', function () {
        var server,
            nodeXML = '<?xml version="1.0" encoding="UTF-8"?><osm>' +
                ...
                'visible="true" timestamp="2009-03-07T03:26:33Z"></node>' +
                '</osm>',
            wayXML = '<?xml version="1.0" encoding="UTF-8"?><osm>' +
                '<node id="1" version="1" changeset="2817006" lat="0" ' +
                'lon="0" visible="true" timestamp="2009-10-11T18:03:23Z"/>' +
                '<way id="1" visible="true" timestamp="2008-01-03T05:24:43Z" ' +
                'version="1" changeset="522559"><nd ref="1"/></way>' +
                '</osm>';
    ...
    describe('#userChangesets', function() {
        ...
        it('loads user changesets', function(done) {
            var changesetsXML = '<?xml version="1.0" encoding="UTF-8"?>' +
                '<osm>' +
                '<changeset id="36777543" user="Steve" uid="1" ' +
                'created_at="2016-01-24T15:02:06Z" closed_at="2016-01-24T' +
                '15:02:07Z" open="false" min_lat="39.3823819" ' +
                'min_lon="-104.8639728" max_lat="39.3834184" ' +
                'max_lon="-104.8618622" comments_count="0">' +
        ...
        it('excludes changesets without comment tag', function(done) {
            var changesetsXML = '<?xml version="1.0" encoding="UTF-8"?>' +
                '<osm>' +
                '<changeset id="36777543" user="Steve" uid="1" ' +
                'created_at="2016-01-24T15:02:06Z" closed_at="2016-01-24' +
                'T15:02:07Z" open="false" min_lat="39.3823819" ' +
                'min_lon="-104.8639728" max_lat="39.3834184" ' +
                'max_lon="-104.8618622" comments_count="0">' +
                '  <tag k="comment" v="Caprice Court has been extended"/>' +
                '  <tag k="created_by" v="iD 2.0.0"/>' +
                '</changeset>' +
                '<changeset id="36777544" user="Steve" uid="1" ' +
                'created_at="2016-01-24T15:02:06Z" closed_at="2016-01-24T' +
                '15:02:07Z" open="false" min_lat="39.3823819" ' +
                'min_lon="-104.8639728" max_lat="39.3834184" ' +
                'max_lon="-104.8618622" comments_count="0">' +
        ...
        describe('#imageryBlacklists', function() {
            it('updates imagery blacklists', function(done) {
                connection.status(function() {
                    ...
                    expect(blacklists).to.deep.equal(
                        ['\.foo\.com','\.bar\.org']);
                    done();
                });
