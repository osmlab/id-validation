describe('iD.svgAreas', function () {
    ...
    it('adds way and area classes', function () {
        var graph = iD.Graph([
                ...
                iD.Way({id: 'w', tags: {building: 'yes'},
                    nodes: ['a', 'b', 'c', 'a']})
            ]);

        surface.call(iD.svgAreas(projection, context), graph,
            [graph.entity('w')], none);
    ...
    describe('z-indexing', function() {
        var graph = iD.Graph([
                ...
                iD.Way({id: 's', tags: {building: 'yes'},
                    nodes: ['a', 'b', 'c', 'd', 'a']}),
                iD.Way({id: 'l', tags: {landuse: 'park'},
                    nodes: ['e', 'f', 'g', 'h', 'e']})
            ]);

        it('stacks smaller areas above larger ones in a single render',
            function () {
                surface.call(iD.svgAreas(projection, context), graph,
                    [graph.entity('s'), graph.entity('l')], none);

                expect(surface.select('.area:nth-child(1)')
                    .classed('tag-landuse-park')).to.be.true;
                expect(surface.select('.area:nth-child(2)')
                    .classed('tag-building-yes')).to.be.true;
            }
        );

        it(
            'stacks smaller areas above larger ones in a single render ' +
                '(reverse)',
            function () {
                surface.call(iD.svgAreas(projection, context), graph,
                    [graph.entity('l'), graph.entity('s')], none);
        ...
        it('stacks smaller areas above larger ones in separate renders',
            function () {
                surface.call(iD.svgAreas(projection, context), graph,
                    [graph.entity('s')], none);
                surface.call(iD.svgAreas(projection, context), graph,
        ...
        it(
            'stacks smaller areas above larger ones in separate renders ' +
                '(reverse)',
            function () {
                surface.call(iD.svgAreas(projection, context), graph,
                    [graph.entity('l')], none);
    ...
    it('renders fill for a multipolygon with tags on the outer way', function(){
        ...
        expect(surface.select('.relation.fill').classed('tag-natural-wood'))
            .to.be.true;
    });

    it('renders no strokes for a multipolygon with tags on the outer way',
        function() {
            var a = iD.Node({loc: [1, 1]}),
                b = iD.Node({loc: [2, 2]}),
                c = iD.Node({loc: [3, 3]}),
