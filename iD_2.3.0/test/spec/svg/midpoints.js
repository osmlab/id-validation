describe('iD.svgMidpoints', function () {
    ...
    it('creates midpoint on segment completely within the extent', function () {
        ...
        surface.call(iD.svgMidpoints(projection, context), graph, [line],
            filter, extent);

        expect(surface.selectAll('.midpoint').datum().loc).to.eql([25, 0]);
    });

    it('doesn\'t create midpoint on segment with pixel length less than 40',
        function () {
            var a = iD.Node({loc: [0, 0]}),
                b = iD.Node({loc: [39, 0]}),
                line = iD.Way({nodes: [a.id, b.id]}),
    ...
    it('doesn\'t create midpoint on segment completely outside of the extent',
        function () {
    ...
    it(
        'creates midpoint on extent edge for segment partially outside of ' +
            'the extent',
        function () {
    ...
    it(
        'doesn\'t create midpoint on extent edge for segment with pixel ' +
            'length less than 20',
        function () {
