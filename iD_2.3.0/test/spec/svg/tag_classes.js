describe('iD.svgTagClasses', function () {
    ...
    it(
        'adds status tag when status in key and value matches "yes" ' +
    ...
    it(
        'adds status tag when status in key and value matches primary ' +
            '(railway=rail + abandoned=railway)',
        function() {
            selection
                .datum(
                    iD.Entity({tags: {railway: 'rail', abandoned: 'railway'}})
                ).call(iD.svgTagClasses());
            expect(selection.attr('class')).to.equal(
                'tag-railway tag-railway-rail tag-status tag-status-abandoned');
        }
    ...
    it(
        'adds primary and status tag when status in key and no primary ' +
            '(abandoned=railway)',
        function() {
            selection
    ...
    it(
        'does not add status tag for different primary tag ' +
            '(highway=path + abandoned=railway)',
        function() {
        selection
                .datum(
                    iD.Entity({tags: {highway: 'path', abandoned: 'railway'}})
                ).call(iD.svgTagClasses());
            expect(selection.attr('class')).to.equal(
    ...
    it(
        'does not add tag-unpaved for highway=track with explicit paved ' +
            'surface tagging',
        function() {
            selection
                .datum(
                    iD.Entity({tags: {highway: 'track', surface: 'asphalt'}})
                ).call(iD.svgTagClasses());
            expect(selection.classed('tag-unpaved')).to.be.false;

            selection
                .datum(
                    iD.Entity({tags: {highway: 'track', tracktype: 'grade1'}})
                ).call(iD.svgTagClasses());
            expect(selection.classed('tag-unpaved')).to.be.false;
    });

    it(
        'adds tag-unpaved for highway=track with explicit unpaved surface ' +
            'tagging',
        function() {
            selection
            ...
            selection
                .datum(
                    iD.Entity({tags: {highway: 'track', tracktype: 'grade3'}})
                ).call(iD.svgTagClasses());
            expect(selection.classed('tag-unpaved')).to.be.true;
        }
    );
    ...
    it(
        'does not add tag-unpaved for other highway types with no surface ' +
            'tagging',
        function() {
            selection
    ...
    it(
        'does not add tag-unpaved for other highway types with explicit ' +
            'paved surface tagging',
        function() {
            selection
                .datum(
                    iD.Entity({tags: {highway: 'tertiary', surface: 'asphalt'}})
                ).call(iD.svgTagClasses());
            expect(selection.classed('tag-unpaved')).to.be.false;
    ...
    it(
        'adds tag-unpaved for other highway types with explicit unpaved ' +
            'surface tagging',
        function() {
            selection
                .datum(
                    iD.Entity({tags: {highway: 'tertiary', surface: 'dirt'}})
                ).call(iD.svgTagClasses());
            expect(selection.classed('tag-unpaved')).to.be.true;
    ...
    it('adds tags based on the result of the `tags` accessor', function() {
        ...
        expect(selection.attr('class'))
            .to.equal('tag-highway tag-highway-primary');
    });
