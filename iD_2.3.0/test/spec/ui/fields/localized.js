describe('iD.uiFieldLocalized', function() {
    ...
    it('adds a blank set of fields when the + button is clicked', function() {
        ...
        expect(selection.selectAll('.localized-lang').nodes().length)
            .to.equal(1);
        expect(selection.selectAll('.localized-value').nodes().length)
            .to.equal(1);
    });
    ...
    it('doesn\'t create a tag when the value is empty', function() {
        ...
        happen.once(selection.selectAll('.localized-lang').node(),
            {type: 'change'});
        happen.once(selection.selectAll('.localized-lang').node(),
            {type: 'blur'});
    });
    ...
    it('doesn\'t create a tag when the name is empty', function() {
        ...
        happen.once(selection.selectAll('.localized-value').node(),
            {type: 'change'});
        happen.once(selection.selectAll('.localized-value').node(),
            {type: 'blur'});
    });
