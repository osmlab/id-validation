describe('iD.uiFieldWikipedia', function() {
    ...
    it('recognizes lang:title format', function() {
        ...
        expect(iD.utilGetSetValue(selection.selectAll('.wiki-lang')))
            .to.equal('English');
        expect(iD.utilGetSetValue(selection.selectAll('.wiki-title')))
            .to.equal('Title');
    });
    ...
    it('sets language, value', function() {
        ...
        expect(spy.getCall(0)).to.have.been.calledWith(
            { wikipedia: undefined, wikidata: undefined });  // lang on change
        expect(spy.getCall(1)).to.have.been.calledWith(
            { wikipedia: undefined, wikidata: undefined });  // lang on blur
        expect(spy.getCall(2)).to.have.been.calledWith(
            { wikipedia: 'de:Title' });   // title on change
        expect(spy.getCall(3)).to.have.been.calledWith(
            { wikipedia: 'de:Title' });   // title on blur
    });
    ...
    it('does not set delayed wikidata tag if graph has changed', function(done){
        ...
        // wikidata not set (t0 + 20ms) after wikipedia="skip" because graph
        // changed
        window.setTimeout(function() {
            expect(context.entity(entity.id).tags.wikidata).to.be.undefined;
        }, 25);

        // wikidata set (t10 + 20ms) after wikipedia="title" because graph
        // unchanged
        window.setTimeout(function() {
            ...
            expect(spy.callCount).to.equal(4);
            expect(spy.getCall(0)).to.have.been.calledWith(
                { wikipedia: 'de:Skip' });   // 'Skip' on change
            expect(spy.getCall(1)).to.have.been.calledWith(
                { wikipedia: 'de:Skip' });   // 'Skip' on blur
            expect(spy.getCall(2)).to.have.been.calledWith(
                { wikipedia: 'de:Title' });  // 'Title' on change +10ms
            expect(spy.getCall(3)).to.have.been.calledWith(
                { wikipedia: 'de:Title' });  // 'Title' on blur   +10ms
            done();
        }, 35);
    });
});
