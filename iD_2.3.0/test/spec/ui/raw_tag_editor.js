describe('iD.uiRawTagEditor', function() {
    ...
    it('creates a pair of empty input elements if the entity has no tags',
        function () {
            element.remove();
            render({});
            expect(element.select('.tag-list').selectAll('input.value')
                .property('value')).to.be.empty;
            expect(element.select('.tag-list').selectAll('input.key')
                .property('value')).to.be.empty;
        }
    );
    ...
    it('adds tags when clicking the add button', function (done) {
        ...
        setTimeout(function() {
            expect(element.select('.tag-list').selectAll('input')
                .nodes()[2].value).to.be.empty;
            expect(element.select('.tag-list').selectAll('input')
                .nodes()[3].value).to.be.empty;
            done();
        }, 20);
    });
    ...
    it('adds tags when pressing the TAB key on last input.value',
        function (done) {
            ...
            var input = d3.select('.tag-list li:last-child input.value')
                    .nodes()[0];
            happen.keydown(d3.select(input).node(), {keyCode: 9});
            setTimeout(function() {
                expect(element.selectAll('.tag-list li').nodes().length)
                    .to.eql(2);
                expect(element.select('.tag-list').selectAll('input')
                    .nodes()[2].value).to.be.empty;
                expect(element.select('.tag-list').selectAll('input')
    ...
    it('does not add a tag when pressing TAB while shift is pressed',
        function (done) {
            expect(element.selectAll('.tag-list li').nodes().length).to.eql(1);
            var input = d3.select('.tag-list li:last-child input.value')
                    .nodes()[0];
            happen.keydown(d3.select(input).node(),
